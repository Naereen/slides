...
\begin{frame}{Outline \uncover<4>{and reference}}
...
\begin{itemize}
\item \emph{``Multi-Player Bandits Revisited''}, Besson \& Kaufmann.
...
\subsection{\hfll{}3.b. Possibly wrong result, not sure yet\hfill{}}
...
\only<2>{
  \begin{figure}[h!]
    \centering
    \includegraphics[height=0.45\textheight]{figures/SIC_MMAB_and_its_regret_bound.png}
    % \caption{\footnotesize{Theorem 1 from their article, regret bound for the SIC-MMAB algorithm (same setting as ours: with sensing).}}
  \end{figure}
}
...
\item
I will work on this more in the close future!
\end{itemize}
...
\item
  ...
  \item
    ...
    \citationright{Ref: [Garivier \& Cappé, 2011]}
  \item
    Choose channel \(A^j(t) = \mathop{\arg\max}\limits_{k} \; \mathrm{UCB}_k^j(t)\),
  \item
    Update \(T_k^j(t+1)\) and \(S_k^j(t+1)\).
...
\citationright{Ref: [Cappé et al, 2013]}
}
...
\subsection{\hfill{}7.f. Comparison with SIC-MMAB and other approaches\hfill{}}


\begin{frame}{Comparison with SIC-MMAB and other approaches}
...
\begin{itemize}\tightlist
  ...
  \item three variants of the SIC-MMAB algorithm (from \textcolor{blue}{\href{https://arxiv.org/abs/1809.08151}{arXiv:1809.08151}}), with \UCB, \klUCB{} and their proposal with {\UCB}-H.
\end{itemize}

\end{frame}


\begin{frame}[plain]{Comparison with other approaches}
...
\caption{\footnotesize{For $M=6$ objects, XXX}}
\end{figure}
...
\caption{\footnotesize{For $M=8$ objects, XXX}}
\end{figure}
...
\caption{\footnotesize{For $M=9$ objects, XXX}}
\end{figure}
...
\begin{itemize}\tightlist
  \item For any algorithm, the \klUCB{} variant is uniformly better than the \UCB{} and {\UCB}-H variant (obviously),
...
\begin{block}{\Selfish}
  ...
  \citationright{Ref: [Bonnefoi \& Besson et al, 2017]}
\end{block}
