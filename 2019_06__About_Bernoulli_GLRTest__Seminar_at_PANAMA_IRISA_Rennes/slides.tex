...
\definecolor{gold}{RGB}{205,110,0}         % rgb(205,110,0)
...
\begin{frame}{Regret of the UCB algorithm and another algorithm}
  ...
  \begin{exampleblock}<2->{For the kl-UCB algorithm: a smaller regret upper-bound}
    \begin{small}
      \[ R_T^{\mathrm{kl}\text{-}\mathrm{UCB}} \leq ( \sum_{\substack{k=1,\dots,K \\ \mu_k < \textcolor{deeppurple}{\mu^*}}} \frac{(\mu_k - \textcolor{deeppurple}{\mu^*})}{\mathrm{kl}(\textcolor{deeppurple}{\mu^*}, \mu_k)} ) \log(T) + o(\log(T)) = \mathcal{O}\left( \alert{\underbrace{C(\mu_1,\dots,\mu_K)}_{\text{Difficulty of the problem}}} \log(T) \right). \]
    \end{small}%
    \begin{footnotesize}
...
\begin{frame}{The break-point detection problem}
  ...
  measurable under $\mathcal{F}_t = \sigma(X_1,\cdots,X_t)$,
...
\begin{frame}{Second result for the BGLR test \dSmiley{}}
  ...
  \begin{block}{Controlling the delay of detection}
      ...
      \begin{small}
      \[ \mathbb{P}_{\textcolor{gold}{\mu_0,\mu_1,\tau}} (\widehat{\tau}_{\delta} \geq \textcolor{gold}{\tau} + u) \leq \exp\left( -\frac{2\textcolor{gold}{\tau} u}{\textcolor{gold}{\tau} + u}\left(\max\left[ 0, \Delta - \sqrt{\frac{\textcolor{gold}{\tau} + u}{2\textcolor{gold}{\tau} u} \beta(\textcolor{gold}{\tau} + u,\delta)} \right]\right)^2 \right) = \mathcal{O}(\searrow(u)). \]
      \end{small}
...
\begin{frame}{Main ideas of our algorithm: BGRL test + kl-UCB index}
  ...
  \begin{exampleblock}{The kl-UCB indexes}
    \begin{itemize}
      ...
      \item $n_k(t)$ counts the the selections
      \item $\widehat{\mu}_k(t)$ is the empirical means of observations since the last reset of arm $k$,
...
\begin{frame}{Two details of our algorithm: BGRL test + kl-UCB index}
...
\begin{footnotesize}
  ...
  \begin{algorithm}[H]
      ...
      \KwData{\emph{Parameters of the algorithm} : $\alpha \in (0, 1)$, $\delta>0$ \tcp*[f]{can depend on $T$ and/or $\Upsilon_T$}}
...
\begin{frame}{Hypotheses of our theoretical analysis}
  ...
  \begin{block}{Assumption}
    ...
    and let $d^{i} = d^{i}(\alpha,\delta) = \lceil \frac{4K}{\alpha(\Delta^{i})^2}\beta(T,\delta) + \frac{K}{\alpha} \rceil$ ($d^0 = 0$).
  ...
  \pause
  $\hookrightarrow$
  The minimum length of the $i$-th sequence depends on the amplitude of the changes at \textcolor{darkred}{the beginning} and \textcolor{darkblue}{the end} of the sequence (\textcolor{darkred}{$\Delta^{i-1}$} and \textcolor{darkblue}{$\Delta^{i}$}).
...
\begin{frame}{Comparison with other state-of-the-art approaches}
  ...
  \begin{block}{CUSUM-UCB \hfill{} \textcolor{gray}{[Liu \& Lee \& Shroff, AAAI 2018]}}
    \begin{itemize}
      \item They obtained $R_T = \mathcal{O}(K \sqrt{T \Upsilon_T \log(T / \Upsilon_T)})$
    \end{itemize}
  \end{block}

  \begin{block}{M-UCB \hfill{} \textcolor{gray}{[Cao \& Zhen \& Kveton \& Xie, AISTATS 2019]}}
    \begin{itemize}
      \item They obtained $R_T = \mathcal{O}(K \sqrt{T \Upsilon_T \log(T)})$
    \end{itemize}
  \end{block}
...
\begin{frame}{Interpretation of the simulations (2/2)}
  ...
  \begin{alertblock}<2->{Time: slow \dSadey{} !}
    But it is too slow!
    Time cost $= \mathcal{O}(K t d_{\max})$
    at every time step $t$,
    % for $K$ arms.
    so $\mathcal{O}(K T^2 d_{\max})$ in total.
    %  and horizon $T$.
  ...
  ($d_{\max} = \max\limits_i \tau^i - \tau^{i+1} =$ duration of the longer stationary sequence, $T \leq (1+\Upsilon_T) d_{\max}$)
...
\begin{frame}{Summary}
  ...
  \begin{itemize}
    \item
    Stationary or \alert{piece-wise stationary} Multi-Armed Bandits problems (MAB)
    \item
    The efficient Bernoulli Generalized Likelihood Ratio test \dCooley{} (BGLR-T)
    \begin{itemize}\tightlist
      \item
