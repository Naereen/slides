...
\begin{frame}{Non stationary MAB problems}
  ...
  \begin{block}{Stationary MAB problems}
    ...
    $\forall t, r(t) \overset{\text{iid}}{\sim} \nu_k = \mathrm{Bernoulli}(\mu_k)$.
  \end{block}

  \begin{alertblock}{Non stationary MAB problems?}
    Arm $k$ gives rewards sampled a (possibly) different same distribution at any time step:
  ...
  \begin{block}{\textbf{Piece-wise stationary} problems!}
    ...
    $\hookrightarrow$ and stationary intervals are supposed to be ``long enough''
  \end{block}
\end{frame}


\begin{frame}{Example of a piece-wise stationary MAB problem}
