...
\definecolor{gold}{RGB}{255,110,0}         % rgb(255,110,0)
...
\begin{frame}{The break-point detection problem}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item[\textcolor{gold}{$\mathcal{H}_1$}] \textcolor{gold}{The distributions have changed mean at a break-point at time $\tau$ \\
...
\begin{frame}{Bernoulli likelihood ratio test}
  ...
  \[ \mathcal{L}(n) = \frac{\sup\limits_{\textcolor{gold}{\mu_0,\mu_1,\tau < n}}\ell(X_1,\dots,X_n ; \textcolor{gold}{\mu_0,\mu_1,\tau})}{\sup\limits_{\textcolor{deeppurple}{\mu_0}}\ell(X_1,\dots,X_n ;\textcolor{deeppurple}{\mu_0})},\]
  %
  where $\ell(X_1,\dots,X_n ; \textcolor{deeppurple}{\mu_0})$ (resp. $\ell(X_1,\dots,X_n ; \textcolor{gold}{\mu_0,\mu_1,\tau})$) is the likelihood of the observations under a model in \textcolor{deeppurple}{$\mathcal{H}_0$} (resp. \textcolor{gold}{$\mathcal{H}_1$}).

  \pause

  \alert{$\hookrightarrow$ High values of this statistic tends to reject \textcolor{deeppurple}{$\mathcal{H}_0$} over \textcolor{gold}{$\mathcal{H}_1$}.}
...
\begin{frame}{Delay of detection}
  ...
  \begin{block}{Delay of detection}
    Let $\mathbb{P}_{\textcolor{gold}{\mu_0,\mu_1,\tau}}$ be a probability model under which $X_t \in[0,1]$ and \textcolor{gold}{$X_t$ a mean $\mu_0$ for all $t \leq \tau$, and $\mu_1$ for all $t > \tau$,
    with $\mu_0 \neq \mu_1$}.
...
\begin{frame}{Simplified form of the regret upper-bound}
  ...
  \begin{block}{Regret upper bound for BGLR + kl-UCB}
    \begin{itemize}
      ...
      %
      \[ R_T =\mathcal{O}\left( \frac{K}{\left(\textcolor{deeppurple}{\Delta^{\text{change}}}\right)^2}\sqrt{T \Upsilon_T \ln(T)} + \frac{(K-1)}{\textcolor{gold}{\Delta^{\text{opt}}}} \Upsilon_T\ln(T) \right),\]
      \item
      ...
      \item
      $\textcolor{gold}{\Delta^{\text{opt}}}$
      \textcolor{gold}{the smallest value of sub-optimality gap on a stationary segment}.
