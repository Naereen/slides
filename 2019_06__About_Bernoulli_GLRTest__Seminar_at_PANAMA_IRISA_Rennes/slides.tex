...
\begin{frame}{The BGLR + kl-UCB algorithm}
  ...
  \begin{figure}[t]
    ...
    \begin{algorithm}[H]
        ...
        % \KwResult{Result}
        % XXX Algorithm
        \textbf{Initialisation : } $\forall i \in \{1,\dots,K\}$, $\tau_i \leftarrow 0$ and $n_i \leftarrow 0$ \\
        \For{$t=1,2,\ldots, T$}{
            \uIf{
            ...
            \Else{
                $A_t \leftarrow \arg\max\limits_{i\in \{1,\dots,K\}} \mbox{UCB}_i(t)$
            ...
            }
            {
                % \uSi{Redémarrages \textbf{globaux}}{
                    $\forall i, \tau_i \leftarrow t$ and $n_i \leftarrow 0 $.\tcp*[f]{reinitialize}
