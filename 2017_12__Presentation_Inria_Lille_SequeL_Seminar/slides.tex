...
\item
  ...
  \citationright{Ref: [Anantharam et al, 1987]} \hook ``price of
...
\item
  ...
  \item
    Compute the index
    \(g_k(t) := \sup\limits_{q \in [a, b]} \left\{ q : \mathrm{kl}\left(\frac{X_k(t)}{N_k(t)}, q\right) \leq \frac{\log(t)}{N_k(t)} \right\}\)
  \item
    Choose channel \(A(t) = \mathop{\arg\max}\limits_{k} \; g_k(t)\),
  \item
    Update \(T_k(t+1)\) and \(X_k(t+1)\).
...
\item
  ...
  \citationright{[Anandkumar et al, 2011]} \pause
\item
  \textbf{Our proposals}: \citationright{[Besson \& Kaufmann, 2017]}
...
\customframefont{\small} \vspace*{-5pt}
