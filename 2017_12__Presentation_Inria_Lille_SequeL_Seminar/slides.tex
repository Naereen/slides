...
\item
  ...
  separately \textcolor<1>{red}{$Y_{A^j(t),t}$} \emph{and}
  \textcolor<1>{blue}{$C^j(t)$}, \newline
...
\item
  ...
  \(\hookrightarrow\) Models licensed protocols (ex. ZigBee), our main
...
\item
  ...
  \newline
  \(\hookrightarrow\) Unlicensed protocols (ex. LoRaWAN), harder to
  analyze!
\end{enumerate}
...
\item
  We study the \textbf{centralized regret} \vspace*{-15pt}
...
\item
  ...
  \(M\) multiplicative factor\ldots{}\newline
   \(\hookrightarrow\) ``price of non-coordination'' \(= M =\) nb of
  player?
\item
