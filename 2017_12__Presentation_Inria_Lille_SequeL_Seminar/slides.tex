...
\item
  \textcolor<2>{RougeFort}{"Sensing"}: first observe
  \(\textcolor<2>{RougeFort}{Y_{A^j(t),t}}\), \emph{then}
  \(\textcolor<2>{Rouge}{C^j(t)}\) only if
  \(\textcolor<2>{RougeFort}{Y_{A^j(t),t}} \neq 0\), \hook Models
  licensed protocols (ex. ZigBee), our main focus. \vspace*{10pt}\pause
\item
  \textcolor<3>{Violet}{"No sensing"}: observe only the joint
  \(\textcolor<3>{Violet}{Y_{A^j(t),t} \times \mathbbm{1}(\overline{C^j(t)})}\),
...
\tightlist
\item
  The first term for sub-optimal arms selections is lower-bounded at
  \(T\to\infty\),
  \(\E_{\mu}[T_k^j(T)] \geq \frac{\log T}{\kl(\mu_k, \mu_M^*)}\), using
  technical information theory tools (Kullback-Leibler divergence,
  entropy),
\item
  And we lower-bound the rest (including collisions) by\ldots{}
  \(0\)\newline
   \(\left(T - \E_{\mu}[T_k(T)]\right) \geq 0\),
  \(\E_{\mu}[\mathcal{C}_k(T)] \geq 0\) : we should be able to do
  better!
\end{itemize}
...
\tightlist
\item
  Centralized \emph{multiple-play} lower-bound is the same without the
  \(M\) multiplicative factor\ldots{} \hook ``price of
  non-coordination'' \(= M =\) nb of player?
\item
...
\item
  ...
  expected number of selections of any sub-optimal arm \(k\).
\item
  Same information-theoretic tools, using a ``change of law'' lemma.
\item
  Improving the state-of-the-art because of our decomposition, not
  because of a new tool.
\end{itemize}
...
\section{\hfill{}5. Multi-player decentralized algorithms\hfill{}}\subsection{\hfill{}5.a. State-of-the-art MP algorithms\hfill{}}
...
\% XXX remove if problem \vspace*{-5pt} \% XXX remove if problem
...
\tightlist
\item
  Bound the expected number of collisions by \(M\) times the nb of
  collisions for non-sitted players,
\item
...
\item
  ...
  constant,
\item
  So most of the times (\$\bigO{T - \log T}), players are sitted, and no
  collision happens when they are all sitted!
\end{enumerate}
