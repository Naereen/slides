...
\begin{frame}{\only<1-4>{Decomposition}\only<5>{\alert{Lower bound}} on regret}
...
R_T(\boldsymbol{\mu}, M, \rho)
&\only<1-4>{=}\only<5>{\alert<5>{\geq}} \alert<2>{\sum_{k \in \Mworst} (\mu_M^* -  \mu_k) \E_{\mu}[T_k(T)]} \\
&\uncover<1-4>{+ \alert<3>{\sum_{k \in \Mbest} (\mu_k -  \mu_M^*) \left(T - \E_{\mu}[T_k(T)]\right)} + \alert<4>{\sum_{k=1}^{K} \mu_k \E_{\mu}[\mathcal{C}_k(T)]}.}
...
\uncover<2>{\alert<2>{\emph{Why bother?}} \klUCB{} is proved to be more
...
\begin{frame}{\RandTopM{} algorithm}
...
\For{$t = 1, \dots, T - 1$}{
    ...
    \eIf{$A^j(t) \notin \TopM(t)$}{
      \eIf(\tcp*[f]{collision}){$C^j(t)$}{
        $A^j(t+1) \sim \mathcal{U} \left(\TopM(t)\right)$
        \tcp*[f]{randomly switch}
        }(\tcp*[f]{\alert{aim arm with smaller UCB at $t-1$}}){
...
\For{$t = 1, \dots, T-1$}{
      ...
      \Else(\tcp*[f]{transition \textcolor{cyan}{$(1)$} or \textcolor{darkgreen}{$(4)$}}){
        ...
        \alert{$s^j(t+1) = \mathrm{True}$} \tcp*[f]{become or stay fixed on a ``chair''}
      }
    Play arm $A^j(t+1)$, get new observations (sensing and collision), \\
    Compute the indices $g^j_k(t+1)$ and set $\TopM(t+1)$ for next step.
}
...
\leq
(\mu_1^* -  \mu_M^*) \left( \textcolor{red}{\sum_{k \in \Mworst} \E_{\mu}[T_k(T)]} + \textcolor{blue}{\sum_{k \in \Mbest} \E_{\mu}[C_k(T)]} \right)
\]\end{small}

So only need to work on both \textcolor{red}{sub-optimal selections} and \textcolor{blue}{collisions}.
\end{block}
