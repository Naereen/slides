...
\begin{frameO}[Is this a good regret rate?]
    ...
    \[
        \cR_\nu(\texttt{ETC},T)  \lesssim  \red \frac{4\sigma^2}{\Delta}\ln\left(T\Delta^2\right)  = \mathcal{O}(\frac{1}{\Delta} \log(T))\black.\]
...
\begin{frameO}[Regret for piece-wise stationary bandits]
  ...
  \begin{orangeblock}{Typical regimes for piece-wise stationary bandits}
    ...
    Currently, state-of-the-art algorithms $\mathcal{A}$ obtain
      ...
      \end{itemize}
    \item
      $\hookrightarrow$ our algorithm GLR + klUCB is state-of-the-art!\\
...
\begin{frameO}[Results on problem 1]
  \centering
...
\begin{frameO}[Multi-players bandits: algorithms]
  ...
  \vspace{0.2cm}
    ...
    % FIXME talk about MCTopM and not rhoRand !

  \alt<2>{
    \textbf{Example:} \orange UCB1 + $\rho^{\text{rand}}$\black. At round $t$ each player
    \begin{itemize}
    ...
    \textbf{Example:} \orange klUCB + MC-Top-M rule\black. At round $t$ each player
    \begin{itemize}
      \item more complicated behavior
  ...
  \textbf{Remarks}:
    \begin{itemize}
      \item $M$ has to be known $\rightarrow $ try to estimate it
...
\begin{frameO}[Multi-players bandits: algorithms]
  ...
  \textbf{Example:} UCB + $\rho^{\text{rand}}$. At round $t$ each player
  \begin{itemize}
    \item has a stored rank $R_{m,t} \in \{1,\dots,M\}$
    \item selects the arm that has the $R_{m,t}$-largest UCB
    \item if a collision occurs, draws a new rank $R_{t+1} \sim \cU(\{1,\dots,M\})$
  \end{itemize}
  ...
  \begin{itemize}
    \item[$\rightarrow$] \orange How does it perform in practice?
  \end{itemize}
...
\begin{frameO}[Where to know more? (3/3)]
    ...
    Play with our Python library for simulations of MAB problems\\
    $\hookrightarrow$ \textbf{SMPyBandits} : \href{https://smpybandits.github.io/}{\textcolor{blue}{\texttt{SMPyBandits.GitHub.io}}}  \& \href{https://GitHub.com/SMPyBandits/}{\textcolor{blue}{\texttt{GitHub.com/SMPyBandits}}} \\
    \begin{itemize}
...
\begin{frameO}[\href{https://smpybandits.github.io/}{\textcolor{blue}{\texttt{SMPyBandits.GitHub.io}}}]
