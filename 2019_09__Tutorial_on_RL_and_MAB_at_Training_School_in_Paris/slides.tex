...
\begin{frameO}[The MOSS algorithm]
    ...
    \alt<2>{\begin{itemize}
        ...
        \end{itemize}}{
        \begin{itemize}
            \item[\ding{220}] matching the worst-case lower bound!
        \end{itemize}}
...
\begin{frame}{Exponential Weighted Forecaster}
  ...
  \[\hat{p}_{t}(k) \propto e^{\eta \left(\sum_{s=1}^{t-1} x_{k,s}\right)}\]
  
  \vspace{-0.3cm}
  
  at round $t$, choose
  \[\red A_t \sim \hat{p}_{t}\black \]
...
\begin{frame}{The EXP3 strategy}
  ...
  \[\red \hat{x}_{k,t} = \frac{x_{k,t}}{\hat{p}_{k,t}}\ind_{(A_t=k)}\]
  
  \vspace{-0.3cm}
  
  satisfies $\bE[\hat{x}_{k,t}] = x_{a,t}$.
  ...
  With $\hat{p}_t$ the probability distribution
  \[\hat{p}_{t}(k) \propto e^{\eta \left(\sum_{s=1}^{t-1} \orange \hat{x}_{k,s}\black\right)}\]
  
  \vspace{-0.3cm}
  
  at round $t$, choose
  \[\red A_t \sim \hat{p}_{t} \]
