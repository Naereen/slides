...
\begin{frameTTnobottom}
    ...
    \begin{textblock*}{12cm}(13mm,50mm)
        ...
        \vspace*{10pt}
        \hspace*{10pt}\emph{Thanks to Émilie Kaufmann for most of the slides material!}
        }
        }}
    \end{textblock*}
...
\begin{frameO}[\alt<2>{The \blue Stochastic \color{white} Multi-Armed Bandit Setup}{The Multi-Armed Bandit Setup}]
    ...
    \textbf{Goal:} Maximize sum of rewards \alt<2>{$\blue \bE\black\left[\sum_{t=1}^T R_t\right]$}{$\sum_{t=1}^T R_t$}.
...
\begin{frameO}[Regret of a bandit algorithm]
    ...
    \begin{orangeblock}{What regret rate can we achieve?}
        \begin{itemize}
            \item[$\rightarrow$] consistency: \black $\cR_\nu(\cA,T) / T \rightarrow_{T\to\infty} 0$
            \item[$\rightarrow$] can we be more precise?
        \end{itemize}
    \end{orangeblock}
\end{frameO}
...
\begin{frameO}[Regret decomposition]
    ...
    \alt<3>{
        ...
        \begin{itemize}
            ...
            \item \dots which requires to try all arms to estimate the values of the $\Delta_a$'s
        \end{itemize}
...
\begin{frameO}[Two naive strategies]
    ...
    \begin{itemize}
        \item \textbf{Idea 1 :}
              \hfill{} \color{red}$\Rightarrow$ EXPLORATION \color{black}
    ...
    \begin{itemize}
        \item \textbf{Idea 2 :} Always trust the empirical best arm
              \hfill{} \color{red}$\Rightarrow$ EXPLOITATION \color{black}
    \end{itemize}
    \begin{orangeblock}{}
...
\begin{frameO}[A better idea: Explore-Then-Commit (\texttt{ETC})]
    ...
    \alt<3>{\hfill{} \red $\rightarrow$ requires a concentration inequality}{}
\end{frameO}
...
\begin{frameO}[A better idea: Explore-Then-Commit (\texttt{ETC})]
    ...
    \hfill{} $\rightarrow$ \red Hoeffding's inequality
\end{frameO}
...
\begin{frameO}[A better idea: Explore-Then-Commit (\texttt{ETC})]
    ...
    \hfill{} $\rightarrow$ \red Gaussian tail inequality
\end{frameO}
...
\begin{frameO}[A better idea: Explore-Then-Commit (\texttt{ETC})]
    ...
    \[\cR_\nu(\texttt{ETC},T) \leq \frac{4\sigma^2}{\Delta}\left[\ln\left(\frac{T\Delta^2}{2}\right) + 1\right] = \mathcal{O}(\frac{1}{\Delta} \log(T)).\]
...
\begin{frameO}[Sequential Explore-Then-Commit (\texttt{S-ETC}) (2 Gaussian arms)]
    ...
    \[
        \cR_\nu(\texttt{S-ETC},T)  \leq  \red \frac{4\sigma^2}{\Delta}\ln\left(T\Delta^2\right)\black \ + C \sqrt{\ln(T)} = \mathcal{O}(\frac{1}{\Delta} \log(T)).\]

    \vspace{-0.3cm}

    \begin{itemize}
        \item[$\rightarrow$] same regret rate, without knowing $\Delta$ \gray [Garivier et al. 2016]
    \end{itemize}
...
\begin{frameO}[Is this a good regret rate?]
    ...
    \begin{itemize}
        \item[$\rightarrow$] \alert{problem-dependent logarithmic} regret bound $\cR_\nu(\text{algo}, T) = \mathcal{O}(\log(T))$.
    \end{itemize}
    ...
    \begin{itemize}
        \item[$\rightarrow$] \alert{problem-independent square-root} regret bound $\cR_\nu(\text{algo}, T) = \mathcal{O}(\sqrt{T})$.
    \end{itemize}
...
\begin{frameO}[Some room for better algorithms?]
    ...
    \begin{itemize}
        \item For two-armed Gaussian bandits, ETC satisfies
              \[
                  \cR_\nu(\mathrm{ETC},T)  \lesssim  \red \frac{4\sigma^2}{\Delta}\black \ln\left(T\Delta^2\right) = \mathcal{O}(\frac{1}{\Delta} \log(T)),\]
    ...
    \begin{itemize}
        \item The Lai and Robbins' lower bound yields, for large values of $T$,
              \[
                  \cR_\nu(\cA,T)  \gtrsim  \red \frac{2\sigma^2}{\Delta}\black \ln\left(T\Delta^2\right) = \Omega(\frac{1}{\Delta} \log(T)),\]
              as $\kl(\mu_1,\mu_2) = \frac{(\mu_1 - \mu_2)^2}{2 \sigma^2}$.
    \end{itemize}

    \begin{itemize}
        \item[$\rightarrow$] Explore-Then-Commit is \alert{not} \blue asymptotically optimal\black.
    \end{itemize}
...
%         \item[$\rightarrow$] Picking  $\blue \cE_T = (N_1(T) > T/2)\black$,
%     \end{itemize}
...
\begin{frameO}[The Lai and Robbins lower bound]
    ...
    \begin{itemize}
        \item[$\rightarrow$] can be extended to cover \blue more general classes of bandit instances \black
    \end{itemize}
...
\begin{frameO}[A simple strategy: $\varepsilon$\texttt{-greedy}]
    ...
    \begin{itemize}
        \item[$\rightarrow$] requires the knowledge of a lower bound on $\Delta_{\min}$.
    \end{itemize}
...
\begin{frameO}[How to build confidence intervals?]
    ...
    \hfill{} $\rightarrow$ tool: \blue concentration inequalities\black
...
\begin{frameO}[Regret of UCB($\alpha$) for bounded rewards]
    ...
    % \begin{itemize}
    %     \item[$\rightarrow$] what we will prove today
    % \end{itemize}
...
%             \item[$\rightarrow$] \red not \black matching the Lai and Robbins lower bound
...
%                 \item[$\rightarrow$] matching the Lai and Robbins lower bound! \blue asymptotically optimal \black
%             \end{itemize}}}
...
\begin{frameO}[The Worst-case Performance of UCB]
    ...
    \end{eqnarray*}
    \begin{itemize}
        \item[$\rightarrow$] not exactly matching the $\blue\Omega(\sqrt{KT})$ lower bound\dots
              but close!
    \end{itemize}
...
%     \begin{itemize}
%         \item[$\rightarrow$] asymptotic optimality beyond Gaussian rewards?
%     \end{itemize}
...
\begin{frameO}[Why is it a UCB?]
    ...
    \alt<3>{\textbf{General case}: follows from
        ...
        \color{red}\Large\danger\normalsize\black Cannot be used directly in a bandit model as \blue the number of observations from each arm is random\black!
            ...
            \begin{itemize}
                \item[$\rightarrow$] upper-confidence bound on $\mu_a$
            \end{itemize}
...
\begin{frameO}[Where do the improvements come from?]
    ...
    \begin{itemize}
        \item[$\rightarrow$] follows from  \red two improvements \black in the previous analysis
    \end{itemize}
...
%     \begin{itemize}
%         \item[$\rightarrow$] not good enough\dots
%     \end{itemize}
...
\begin{frameO}[\klUCB{} beyond exponential families]
    ...
    \begin{itemize}
        \item \klUCB{} \blue can be used for arbitrary rewards in $[0,1]$ \black with
              \begin{itemize}
                  \item[$\rightarrow$] the Gaussian divergence $\kl(x,y) = 2(x-y)^2$ (UCB)
                  \item[$\rightarrow$] the Bernoulli divergence $\kl(x,y) = \KL(\cB(x),\cB(y))$
              \end{itemize}
    ...
    \begin{itemize}
        \item variants of kl-UCB for \blue other types of parametric reward distributions \black
              \begin{itemize}
                  \item[$\rightarrow$] distribution with a finite support \hfill \gray[Maillard et al. 11][Cappé et al. 13]\black

                  \item[$\rightarrow$] exponential family with $d > 1$ parameters \hfill  \gray [Maillard, 17]
              \end{itemize}
    \end{itemize}
    ...
    \begin{itemize}
        \item variants that \blue do not exploit parametric assumptions \black that obtain better guarantees for arbitrary rewards
              \begin{itemize}
                  \item[$\rightarrow$] DMED, IMED \hfill \gray [Honda and Takemura, 10][Honda and Takemura, 16] \black
                  \item[$\rightarrow$] empirical KL-UCB for bounded rewards \hfill \gray[Cappé et al. 13]\black
              \end{itemize}
    \end{itemize}
...
%     \alt<2>{\begin{itemize}
%             \item[$\rightarrow$] \blue far from optimal \black in a problem-dependent sense
%         \end{itemize}}{
%         \begin{itemize}
%             \item[$\rightarrow$] matching the worse-case lower bound in $\Omega(\sqrt{KT})$ !
%         \end{itemize}}
...
\begin{frameO}[Example: Bernoulli bandits]
    ...
    \begin{itemize}
        ...
        \item[$\rightarrow$] \underline{posterior distribution}:
              \vspace{-0.6cm}
...
%     \begin{itemize}
%         \item[$\rightarrow$] looks like the \blue \klUCB{} index\black, with a \blue different exploration rate\black\dots
%     \end{itemize}
...
%     \begin{itemize}
%         \item[$\rightarrow$] How? Optimality? For which distributions ?
%     \end{itemize}
    ...
    \begin{itemize}[<+->]
        ...
        \item (centralized) multiple-actions
              \begin{itemize}
                  ...
                  \item combinatorial : choose a subset of arms $S \sub \{1,\dots,K\}$ (large space)
              \end{itemize}
        \item non stationary
              \begin{itemize}
...
%   \begin{itemize}
%     \item[$\rightarrow$] may be a bad idea\dots
%   \end{itemize}
...
%   \textbf{Strategies}:
%   \begin{itemize}
%     \item static strategy playing a single arm $\rightarrow$ average reward $1/2$
...
%   \begin{itemize}
%     \item[$\rightarrow$] regret with respect to the best static action is no longer (always) the right notion
%   \end{itemize}
...
\begin{frameO}[Piece-wise stationary bandits]
    ...
    \begin{orangeblock}{\textbf{Piece-wise stationary} problems!}
        $\hookrightarrow$ focus on the easier case when there are at most $Y_T = o(\sqrt{T})$ intervals on which the means are all stationary.
    \end{orangeblock}
\end{frameO}
...
\begin{frameO}[Example of a piece-wise stationary MAB problem]
    ...
    \begin{center}
        \includegraphics[width=0.95\textwidth]{Problem_1.pdf}
    \end{center}
\end{frameO}
    ...
    \begin{orangeblock}{Typical regimes for piece-wise stationary bandits}
        \begin{itemize}
            ...
            \item
                  ...
                  \end{itemize}
            \item
                  $\hookrightarrow$ our algorithm \textcolor{blue}{BGLR + klUCB} is state-of-the-art!\\
...
\begin{frameO}[Results on a piece-wise stationary MAB problem]
    ...
    \begin{center}
        \includegraphics[width=0.85\textwidth]{regret_problem1.png}
    \end{center}

    \textcolor{blue}{BGLR + klUCB} achieves the best performance (among non-oracle algorithms) !
\end{frameO}
...
\begin{frameO}[Multi-players bandits: algorithms]
    ...
    \only<1>{
        \begin{orangeblock}{\textbf{Example:} UCB1 + $\rho^{\text{rand}}$. At round $t$ each player}
            \begin{itemize}
                ...
                \item \textbf{Early references}: \color{gray} [Liu and Zhao, 10] [Anandkumar et al., 11]
    ...
    \only<2->{
        \begin{orangeblock}{\textbf{Example:} our algorithm \textcolor{blue}{klUCB index + MC-Top-M rule}}
            \begin{itemize}
                \only<2->{
    ...
    \only<3>{
        \textbf{Remarks}:
        \begin{itemize}
            \item $M$ has to be known $\rightarrow$ but it is possible to estimate it on the run
            \item does not handle an evolving number of devices (entering/leaving the network)
...
\begin{frameO}[Results on a multi-player MAB problem]
    ...
    \centering
    \includegraphics[height=0.75\textheight]{all_RegretCentralized_loglog____env1-1_6747959631471381163.pdf}
    \caption{\footnotesize{For $M=6$ objects, MC-TopM and Rand-TopM largely outperform SIC-MMAB and $\rho^{\text{rand}}$. \textcolor{blue}{MCTopM + klUCB} achieves the best performance (among decentralized algorithms) !}}
...
\begin{frameO}[Where to know more? (1/3)]
...
\begin{frameO}[Where to know more? (2/3)]
...
\begin{frameO}[Where to know more? (3/3)]
