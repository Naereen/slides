...
\setbeamertemplate{footline}{ \hspace{5em} \textcolor{white} {Lilian Besson - Intro to MAB \hfill 23 September, 2019}\hspace{2em}\null \vspace*{3pt}}
\begin{frame}{#1}
\BODY
\end{frame}
}
...
\setbeamertemplate{footline}{\hspace{2cm} \raisebox{2.5ex}
	{{Lilian Besson - Intro to MAB}}\hfill
	\raisebox{2.5ex}
...
\begin{frameTT}
    ...
    \begin{textblock*}{12cm}(13mm,50mm)
        ...
        {\Large
            Training School\\
            on Machine Learning for Communications\\
...
\setbeamertemplate{footline}{\hspace{2cm} \raisebox{2.5ex}
    {\textcolor{white}{Lilian Besson - Intro to MAB}}\hfill
    \raisebox{2.5ex}
...
\begin{frameO}[Sequential resource allocation]
    ...
    \begin{itemize}
        \item $K$ treatment for a given symptom (with unknown effect)
...
\begin{frameO}[Dynamic allocation of computational resource]
...
\begin{frameO}[Outline of the RLSS Bandit Class]
...
\begin{frameO}[\alt<2>{The \blue Stochastic \color{white} Multi-Armed Bandit Setup}{The Multi-Armed Bandit Setup}]
    ...
    \textbf{Goal:} Maximize \alt<2>{$\blue \bE\black\left[\sum\limits_{t=1}^T R_t\right]$}{$\sum_{t=1}^T R_t$}.


\end{frameO}

\begin{frameO}[Discover by playing this demo]

  \begin{center}
    % \centering
    \includegraphics[width=0.85\textwidth]{example_of_a_5_arm_bandit_problem.png}
  \end{center}
...
\begin{frameO}[Two naive strategies]
    ...
    \[\cR_\nu(\cA,T) = \left(\frac{1}{K}\sum_{a : \mu_a > \mu_\star} \Delta_a\right) T \]
    ...
    \[\hspace{1.5cm}\cR_\nu(\cA,T) \geq  (1-\mu_1)\times \mu_2 \times (\mu_1 - \mu_2) T \]
...
\begin{frameO}[Behind the lower bound: a change of distribution]
    ...
    arm $a$ is optimal under $\bm \lambda =(\mu_1,\dots,\mu_{a-1},\blue\mu_1 + \epsilon\black,\mu_{a+1},\dots,\mu_K)$

    \pause
    \alt<3>{\[\red \bE_{\bm\mu}[N_a(T)] \gtrsim \frac{\ln(T)}{\kl(\mu_a,\mu_\star + \epsilon)}\]\vspace{-0.4cm}\begin{center}for large values of $T$                                                                                                                                                                                        \end{center}
    }{
    \begin{itemize}
        \item[\ding{220}]
              $\sum_{a=1}^K \bE_{\bm \mu}[N_a(T)]\times \kl(\mu_a,\lambda_a) = \red\bE_{\bm \mu}[N_a(T)] \kl(\mu_a,\mu_1+\epsilon)$ \\
...
\begin{frameO}[A simple strategy: $\epsilon$-greedy]

    The $\epsilon$-greedy rule \color{gray} [Sutton and Barton, 98] \black is the simplest way to alternate exploration and exploitation.

    \begin{orangeblock}{$\epsilon$-greedy strategy}
        At round $t$,
        \begin{itemize}
            \item with probability $\epsilon$
                  \[A_t \sim \cU(\{1,\dots,K\})\]
            \item with probability $1-\epsilon$
                  \[A_t = \argmax_{a = 1,\dots,K} \ \hat{\mu}_a(t).\]
        \end{itemize}
    \end{orangeblock}

    \begin{itemize}
        \item[\ding{220}] \underline{Linear regret}: $\cR_\nu\left(\epsilon\texttt{-greedy},T\right) \geq \epsilon\frac{K - 1}{K}\Delta_{\min} T$.
...
\begin{frameO}[A simple strategy: $\epsilon$-greedy]
    ...
    \begin{orangeblock}{$\epsilon_t$-greedy strategy}
        At round $t$,
        \begin{itemize}
            \item with probability $\blue\epsilon_t :=\min \left(1, \frac{K}{d^2t}\right)$
                  \[A_t \sim \cU(\{1,\dots,K\})\]

                  \vspace{-0.4cm}

            \item with probability $1-\epsilon_t$
    ...
    \begin{alertblock}{Theorem \gray [Auer et al. 02]\black}
        If $\red 0 < d \leq \Delta_{\min}$, $\cR_\nu\left(\epsilon_t\texttt{-greedy},T\right) = O\left(\frac{K\ln(T)}{d^2}\right)$.
    \end{alertblock}
...
\begin{frameO}[UCB for Gaussian distributions]
    ...
    \[\cR_{\nu}(\texttt{ISM},T) \leq (1+\epsilon)\!\!\!\!\!\!\!\!\underbrace{\blue\sum_{a : \mu_a < \mu_\star} \frac{2}{\ln\left(1+ \frac{\Delta_a^2}{\sigma_a^2}\right)}\black}_{\substack{\text{optimal constant}\\\text{(Burnetas and Katehakis lower bound)}}}\!\!\!\!\!\!\!\!\ln(T) + O_\epsilon(\ln\ln(T)).\]
...
\begin{frameTI}
    \begin{center}
        {\textcolor{white} {\Huge \textsc{Worse-case Optimality} }}
    \end{center}
    \vspace*{-4pt}
\end{frameTI}
...
\begin{frameO}[Theoretical results in the Bernoulli case]
    ...
    \begin{orangeblock}{Theorem \color{gray}[K.,Capp\'e,Garivier 2012]}
        ...
        \vspace{-0.3cm}
        \[\mathbb{E}_{\bm\mu}[N_a(T)]\leq \frac{1+\epsilon}{\kl(\mu_a,\mu_\star)}\ln(T) + o_{\epsilon,c}\left(\ln(T)\right).\]
        %\begin{align*}
        % &  \bE[N_a(T)] \leq \frac{1+\epsilon}{d(\mu_a,\mu_1)}\ln(T) +
        % \sqrt{\ln T + 5\ln\ln T} \sqrt{\frac{2\pi(1+\epsilon)^3d'(\mu_a,\mu_1)^2}{d(\mu_a,\mu_1)^3}} \\
        %& \ \ \ \ \ \ \ \ \ + \left(\frac{1+\epsilon}{d(\mu_a,\mu_1)} +
        %\frac{2e+3}{1-\mu_1}\right)\ln \ln T + 27 +  2(1+\epsilon)^2 \left(\frac{d'(\mu_a,\mu_1)}{d(\mu_a,\mu_1)}\right)^2.
        %\end{align*}
    \end{orangeblock}
...
\begin{frameO}[Beyond Bernoulli bandits]
    ...
    \begin{orangeblock}{Theorem \gray [K. 17]}
        ...
        \[\red\bE_{\bm \mu}[N_a(T)] \leq \frac{1+\epsilon}{\kl(\mu_a,\mu_\star)}\ln(T) + O_{\epsilon}(\sqrt{\ln(T)})\black.\]
...
\begin{frameO}[Thompson Sampling is asymptotically optimal]
    ...
    \begin{orangeblock}{Problem-dependent regret}
        \[\forall \epsilon>0, \ \ \bE_{\bm\mu}[N_a(T)] \leq (\color{red}1+\epsilon\color{black})\frac{1}{\color{red}\kl(\mu_a,\mu_\star)\color{black}}\ln(T) + o_{\mu,\epsilon}(\ln(T)).\]
...
\begin{frameO}[Going back to "Follow the leader"]
    ...
    \begin{orangeblock}{Properties}
        \begin{itemize}
            ...
            \item A variant ($\epsilon$-greedy) performs ok if well-tuned \gray[Auer et al, 2002]\black.
        \end{itemize}
    \end{orangeblock}
    ...
    {\small
        \begin{itemize}
            ...
            \item[2011] An empirical evaluation of Thompson Sampling: \red an efficient algorithm\black, beyond simple bandit models
                  ...
                  \gray [Chapelle and Li, 2011]\black
...
\begin{frameO}[A Puzzling strategy]
    ...
    \begin{center}
        \large{\textcolor{red}{B}est  \textcolor{red}{E}mpirical \textcolor{red}{S}ub-sampling \textcolor{red}{A}verage}
...
\begin{frameO}[Good practical performance ($T = 20,000$, $N=50,000$)]
    ...
    \begin{center}
        \begin{tabular}{|c|ccccc|}
            \hline
                      & \textcolor{red}{BESA} & kl-UCB & kl-UCB+    & TS    & Others  \\
            \hline
            Regret    & {\bf 74.4}            & 121.2  & {\bf 72.8} & 83.4  & 100-400 \\
            Beat BESA & -                     & 1.6\%  & 35.4\%     & 3.1\% &         \\
            Run Rime  & 13.9X                 & 2.8X   & 3.1X       & X     &         \\
...
\begin{frameO}[Good practical performance ($T = 20,000$, $N=50,000$)]
    ...
    \begin{center}
        \begin{tabular}{|c|ccccc|}
            \hline
                      & \textcolor{red}{BESA} & KL-UCB-exp & UCB-tuned & FTL 10 & Others      \\
            \hline
            Regret    & {\bf	53.3}             & 65.7       & 97.6      & 306.5  & 60-110,120+ \\
            Beat BESA & -                     & 5.7\%      & 4.3\%     & -      &             \\
            Run Rime  & 6X                    & 2.8X       & X         & -      &             \\
...
\begin{frameO}[Good practical performance ($T = 20,000$, $N=50,000$)]
    ...
    \begin{center}
        ...
        \begin{tabular}{|c|cccc|}
            \hline
                      & \textcolor{red}{BESA} & KL-UCB-Poisson & kl-UCB & FTL 10 \\
            \hline
            Regret    & \bf{19.4}             & 25.1           & 150.6  & 144.6  \\
            Beat BESA & -                     & 4.1\%          & 0.7\%  & -      \\
            Run Rime  & 3.5X                  & 1.2X           & X      & -      \\
...
\begin{frameO}[Where to know more?]
    ...
    \normalsize \href{https://tor-lattimore.com/downloads/book/book.pdf}{{(https://tor-lattimore.com/downloads/book/book.pdf)}}
