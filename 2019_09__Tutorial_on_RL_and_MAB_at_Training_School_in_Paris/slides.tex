...
\begin{frameO}[Dynamic allocation of computational resources]
    ...
    \textbf{Numerical experiments} (Gaussian processes + bandits for black-box optimization)
...
\begin{frameO}[Outline of this talk]
    ...
    \begin{itemize}
      ...
      \item Performance measure and first strategies
      \item Best possible regret? Lower bounds
      \item Mixing Exploration and Exploitation
      \item The Optimism Principle and Upper Confidence Bounds Algorithms
...
\begin{frameO}[Regret of a bandit algorithm]
    ...
    \begin{orangeblock}{What regret rate can we achieve?}
        \begin{itemize}
            \item[\ding{220}] consistency: \black $\cR_\nu(\cA,T) / T \rightarrow_{T\to\infty} 0$
...
\begin{frameO}[Two naive strategies]
    ...
    \begin{itemize}
        \item \textbf{Idea 2 :} Always trust the empirical best arm $A_{t+1}=\underset{a \in \{1,\dots,K\}}{\text{argmax}} \ \blue \widehat{\mu}_a(t)$
    \end{itemize}
    \begin{orangeblock}{}
        where
...
\begin{frameO}[A better idea: Explore-Then-Commit (\texttt{ETC})]
    ...
    \underline{Analysis for $K=2$ arms}. $\mu_1 > \mu_2$,  $\textcolor{blue}{\Delta := \mu_1 - \mu_2}$.
...
\begin{frameO}[Is this a good regret rate?]
    ...
    \begin{itemize}
        \item[\ding{220}] \alert{problem-dependent logarithmic} regret bound
    \end{itemize}
    ...
    \begin{itemize}
        \item[\ding{220}] \alert{problem-independent square-root} regret bound $R(T) = \mathcal{O}(\sqrt{T})$.
    \end{itemize}
...
\begin{frameO}[A simple strategy: $\varepsilon$\texttt{-greedy}]
    ...
    \begin{orangeblock}{Theorem \gray [Auer et al. 02]\black}
        If $\red 0 < d \leq \Delta_{\min}$, $\cR_\nu\left(\varepsilon_t\texttt{-greedy},T\right) = \mathcal{O}\left(\frac{K\ln(T)}{d^2}\right)$.
    \end{orangeblock}
...
\begin{frameO}[How to build confidence intervals?]
    ...
    \begin{alertblock}{Hoeffding inequality}
        ...
        \[\alt<2->{\bP\left(\frac{Z_1 + \dots + Z_s}{s}  \leq \mu - x \right)  \leq e^{-\frac{sx^2}{2\sigma^2}}}{\bP\left(\frac{Z_1 + \dots + Z_s}{s}  \geq \mu + x \right)  \leq e^{- s x^2 / (2\sigma^2)}}\]
    \end{alertblock}
    ...
    \alt<3>{\color{red}\Large\danger\normalsize\black Cannot be used directly in a bandit model as \blue the number of observations $s$ from each arm is random\black!}{\vspace{-0.3cm}
        ...
        \begin{itemize}
            ...
            \item $\nu_a = \cN(\mu_a,\sigma^2)$: $\sigma^2$ sub-figGaussianElimination
        \end{itemize}
    }
...
\begin{frameO}[How to build confidence intervals?]
    ...
    \begin{orangeblock}{Hoeffding inequality + union bound}
        $$\bP\left(\mu_a \leq \red \widehat{\mu}_a(t) + \sigma\sqrt{\frac{\beta\ln(t)}{N_a(t)}} \black \right) \geq 1 - \frac{1}{t^{\frac{\beta}{2} -1}}$$
    \end{orangeblock}
    ...
    \begin{align*}
         & \bP\left(\mu_a > \widehat{\mu}_a(t) + \sigma\sqrt{\frac{\beta\ln(t)}{N_a(t)}} \black \right) \leq  \bP\left(\exists s \leq t : \mu_a > \widehat{\mu}_{a,s} + \sigma\sqrt{\frac{\beta\ln(t)}{s}} \black\right) \\
         & \leq \sum_{s=1}^t \bP\left(\widehat{\mu}_{a,s} < \mu_a - \sigma\sqrt{\frac{\beta\ln(t)}{s}}\right) \leq \sum_{s=1}^t \frac{1}{t^{\beta/2}} = \frac{1}{t^{\beta/2 - 1}}.
    \end{align*}
...
\begin{frameO}[The Worst-case Performance of UCB]
    ...
    \end{eqnarray*}
    \begin{itemize}
        \item[\ding{220}] not exactly matching the $\blue\Omega(\sqrt{KT})$ lower bound...
    \end{itemize}
...
\begin{frameO}[The MOSS algorithm]
    ...
    \alt<2>{\begin{itemize}
        ...
        \end{itemize}}{
        \begin{itemize}
            \item[\ding{220}] matching the worse-case lower bound in $\Omega(\sqrt{KT})$ !
        \end{itemize}}
...
\begin{frame}{A restless bandit example}
  ...
  \begin{itemize}
    \item[\ding{220}] may be a bad idea...
  \end{itemize}
...
\begin{frame}{Optimal strategies?}
  ...
  \begin{itemize}
    \item[\ding{220}] regret with respect to the best static action is no longer (always) the right notion
  \end{itemize}
...
\section{Bandits for multiple devices}
...
\begin{frame}{Multi-players bandits: algorithms}
  ...
  \begin{itemize}
    \item[\ding{220}] \orange How does it perform in practice?
  \end{itemize}
