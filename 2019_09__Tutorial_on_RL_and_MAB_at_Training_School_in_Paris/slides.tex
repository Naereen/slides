...
\begin{frameO}[Dynamic channel selection]
    ...
    \begin{itemize}
        ...
        \item In which channel should a radio device send a packet,n based on the quality of its previous communications?
              \pause
...
\begin{frameO}[A better idea: Explore-Then-Commit (\texttt{ETC})]
    ...
    \[\cR_{\bm\nu}(\texttt{ETC},T) \leq \frac{4\sigma^2}{\Delta}\left[\ln\left(\frac{T\Delta^2}{2}\right) + 1\right] = \mathcal{O}\left(\frac{1}{\Delta} \log(T)\right).\]
...
\begin{frameO}[Sequential Explore-Then-Commit (\texttt{S-ETC}) (2 Gaussian arms)]
    ...
    \[\hspace{-1cm}\blue \tau = \inf \left\{t \in \N : |\widehat{\mu}_1(t) - \widehat{\mu}_2(t) | > \sqrt{\frac{8\sigma^2\ln(T/t)}{t}}\right\}  \]
    ...
    \[
        \cR_{\bm\nu}(\texttt{S-ETC},T)  \leq  \red \frac{4\sigma^2}{\Delta}\ln\left(T\Delta^2\right)\black \ + C \sqrt{\ln(T)} = \mathcal{O}\left(\frac{1}{\Delta} \log(T)\right).\]
...
\begin{frameO}[Is this a good regret rate?]
    ...
    \[
        \cR_{\bm\nu}(\texttt{ETC},T)  \lesssim  \red \frac{4\sigma^2}{\Delta}\ln\left(T\Delta^2\right)  = \mathcal{O}\left(\frac{1}{\Delta} \log(T)\right)\black.\]
...
\begin{frameO}[The Lai and Robbins lower bound]
    ...
    \begin{orangeblock}{Kullback-Leibler divergence}
        ...
        \[
            \red \mathrm{kl}(\mu,\mu')  \black : =   \alt<3>{\mu\ln \left(\frac{\mu}{\mu'}\right) + (1-\mu) \ln \left(\frac{1-\mu}{1-\mu'}\right) \ \ \ (\text{Bernoulli bandits})}{\alt<2>{\frac{(\mu - \mu')^2}{2\sigma^2} \ \ \ (\text{Gaussian bandits with variance } \sigma^2)}{\text{KL}\left(\nu_\mu,\nu_{\mu'}\right) =\bE_{X \sim \nu_{\mu}}\left[\ln \frac{d\nu_{\mu}}{d\nu_{\mu'}}(X)\right]}}
        \]
    ...
    \begin{orangeblock}{Theorem \color{gray} [Lai and Robbins, 1985]}
        ...
        \[
            \color{red}\mu_a<\mu_\star \Longrightarrow \liminf_{T\Longrightarrow\infty}\frac{\bE_{\bm \mu}[N_{a}(T)]}{\ln T}\geq \frac{1}{\kl(\mu_a,\mu_\star)}.\color{black}
        \]
...
\begin{frameO}[Some room for better algorithms?]
    ...
    \begin{itemize}
        \item For two-armed Gaussian bandits, ETC satisfies
              \[
                  \cR_{\bm\nu}(\mathrm{ETC},T)  \lesssim  \red \frac{4\sigma^2}{\Delta}\black \ln\left(T\Delta^2\right) = \mathcal{O}\left(\frac{1}{\Delta} \log(T)\right),\]
    ...
    \begin{itemize}
        \item The Lai and Robbins' lower bound yields, for large values of $T$,
              \[
                  \cR_{\bm\nu}(\cA,T)  \gtrsim  \red \frac{2\sigma^2}{\Delta}\black \ln\left(T\Delta^2\right) = \Omega\left(\frac{1}{\Delta} \log(T)\right),\]
...
\begin{frameO}[A simple strategy: $\varepsilon$\texttt{-greedy}]
    ...
    \begin{orangeblock}{Theorem \gray [Auer et al. 02]\black}
        If $\red 0 < d \leq \Delta_{\min}$, $\cR_{\bm\nu}\left(\varepsilon_t\texttt{-greedy},T\right) = \mathcal{O}\left(\frac{1}{d^2} K\ln(T)\right)$.
    \end{orangeblock}
...
\begin{frameO}[The optimism principle]
    ...
    \begin{figure}
        ...
        \caption{Confidence intervals on the means after $t$ rounds, for $K=4$ arms}
    \end{figure}
...
\begin{frameO}[How to build confidence intervals?]
    ...
    \begin{orangeblock}{Hoeffding inequality + union bound}
        \[ \bP\left(\mu_a \leq \red \widehat{\mu}_a(t) + \sigma\sqrt{\frac{\alpha\ln(t)}{N_a(t)}} \black \right) \geq 1 - \frac{1}{t^{\frac{\alpha}{2} -1}} \]
    \end{orangeblock}
    ...
    \begin{align*}
         & \bP\left(\mu_a > \widehat{\mu}_a(t) + \sigma\sqrt{\frac{\alpha\ln(t)}{N_a(t)}} \black \right) \leq  \bP\left(\exists s \leq t : \mu_a > \widehat{\mu}_{a,s} + \sigma\sqrt{\frac{\alpha\ln(t)}{s}} \black\right) \\
         & \leq \sum_{s=1}^t \bP\left(\widehat{\mu}_{a,s} < \mu_a - \sigma\sqrt{\frac{\alpha\ln(t)}{s}}\right) \leq \sum_{s=1}^t \frac{1}{t^{\alpha/2}} = \frac{1}{t^{\alpha/2 - 1}}.
    \end{align*}
...
\begin{frameO}[A first UCB algorithm]
    ...
    where
    \[\blue \mathrm{UCB}_a(t) =\black \underbrace{\blue \widehat{\mu}_{a}(t)}_{\text{exploitation term}} + \underbrace{\blue\sqrt{\frac{\alpha\ln(t)}{N_a(t)}}}_{\text{exploration bonus}}.\]
...
%     \begin{eqnarray*}
%         \sum_{t=0}^{T-1}\bP(\UCB_1(t) \leq \mu_1) & \leq & 1 + \sum_{t=1}^{T-1} \bP\left(\widehat{\mu}_1(t) + \sqrt{\frac{\alpha \ln(t)}{N_1(t)}} \leq \mu_1\right) \\
...
%     \[\mathrm{LCB}_a(t) = \widehat{\mu}_{a}(t) - \sqrt{\frac{\alpha \ln t}{N_a(t)}}.\]
...
%             \begin{eqnarray*}
%                 &\Longrightarrow &(\mu_1 - \mu_a) \leq 2 \sqrt{\frac{\alpha \ln(T)}{N_a(t)}} \\
%                 & \Longrightarrow & \ N_a(t) \leq \frac{4\alpha}{(\mu_1 - \mu_a)^2}\ln(T)
%             \end{eqnarray*}
%         \end{column}
%         \begin{column}{4.5cm}
%             \begin{center}
...
%     \[\UCB_a(t) = \widehat{\mu}_a(t) + \sqrt{\frac{2\sigma^2(\ln(t) + c\ln\ln(t))}{N_a(t)}}\]
...
%     \begin{itemize}
%         \item UCB worst-case regret:  $\mathcal{O}(\blue\sqrt{KT \ln(T)})$
%     \end{itemize}
%     \begin{eqnarray*}
%         \cR_{\bm\nu}(\texttt{UCB},T) & = &\sum_{a=1}^K \textcolor{red}{\Delta_a\sqrt{\bE[N_a(T)]}}\sqrt{\bE[N_a(T)]} \\
%         &=&\sum_{a=1}^K \mathcal{O}(\red{\sqrt{\ln(T)}}\black)\sqrt{\bE[N_a(T)]}\\
%         &\leq& K\sqrt{\frac{1}{K} \sum_{a}\bE[N_a(T)]}\mathcal{O}(\sqrt{\ln(T)})\\
%         &=& \mathcal{O}(\sqrt{KT \ln(T)})
%     \end{eqnarray*}
%     \begin{itemize}
...
%     \[A_{t+1} = \argmax_{a=1,\dots,K} \ \red  \hat\mu_{a}(t) + \sqrt{\frac{2{\widehat{\sigma}_a(t)} \ln t^3}{N_a(t)}} + \frac{7\ln t^3}{3{N_a(t)}}\]
...
\begin{frameO}[The $\mathrm{kl}$-UCB index]
    ...
    \begin{orangeblock}{}
        ...
        \vspace{-0.5cm}
        \[\UCB_{a}(t)=\max\left\{q : \red \kl\black\left(\widehat{\mu}_a(t),q\right) \leq \frac{\ln(t)+c\ln\ln(t)}{N_a(t)}\right\},
        \]
    \end{orangeblock}
...
\begin{frameO}[Why is it a UCB?]
    ...
    \alt<3>{\textbf{General case}: follows from
        ...
        \color{red}\Large\danger\normalsize\black Cannot be used directly in a bandit model as \blue the number of observations from each arm is random\black!
            ...
            We recover
            \[\blue\UCB_a(t) = \widehat{\mu}_a(t) + \sqrt{\frac{2\sigma^2\left(\ln(t) + c \ln\ln(t)\right)}{N_a(t)}}\black\]
            \begin{itemize}
...
%     \begin{eqnarray*}
%         \bP(\UCB_1(t) \leq \mu_1) & = & \bP\left( N_1(t)\times \kl^+\!\!\left(\widehat{\mu}_1(t),\mu_1\right) > \ln(t) + c\ln\ln(t)\right) \\
%         & \leq & \bP\left(\exists s \leq t : s \times \kl^+(\widehat{\mu}_{1,s},\mu_1) > \ln(t) + c\ln\ln(t)\right)
%     \end{eqnarray*}

%     \alt<3>{\textbf{Second idea:} \red peeling trick \black
%     \begin{alertblock}{Lemma \gray[Garivier and Cappé, 2011]}
%         \[\bP\left(\exists s \leq t : s \times \kl^+\left(\widehat{\mu}_{1,s},\mu_1\right) > \gamma \right) \leq e \lceil\gamma \ln(t)\rceil  e^{-\gamma}.\]
%     \end{alertblock}

%     \vspace{-0.3cm}

%     \begin{eqnarray*}
%         \bP(\UCB_1(t) \leq \mu_1) & = & \mathcal{O}\left( \frac{\ln^2(t)}{t\ln^c(t)}\right)
%     \end{eqnarray*}
...
%     \begin{align*}
%         \hspace{-0.4cm}\bP(\UCB_1(t) \leq \mu_1) & \!\leq\!\!\sum_{k=1}^{\frac{\ln(t)}{\ln(1+\eta)}} \bP\left( \exists s \in \cI_k , s\times \kl^+\!\!\left(\widehat{\mu}_{1,s},\mu_1\right)\! > \!\ln(t) + c\ln\ln(t)\right)                                                                                             \\
%                                                  & \leq \!\!  \sum_{k=1}^{\frac{\ln(t)}{\ln(1+\eta)}} \underbrace{\bP\left( \exists s \in \cI_k , s\times \kl^+\!\!\left(\widehat{\mu}_{1,s},\mu_1\right)\! > \!\ln(t_{k}) + c\ln\ln(t_{k})\right)}_{\substack{\text{deviation of } \widehat{\mu}_{1,s} \text{ from its mean} \\\ \blue \text{uniformly over } s\in\cI_k}} \\
%     \end{align*}
...
%     \begin{eqnarray*}
%         \bP(\UCB_1(t) \leq \mu_1) & = & \sum_{s=1}^t \bP\left( s\times \kl^+\!\!\left(\widehat{\mu}_{1,s},\mu_1\right) > \ln(t) + c\ln\ln(t)\right) \\
...
% {\vspace{-0.5cm}
%
%  \[\color{gray} \text{[Cappé et al. 13]}\black: \ \ \ \bE_{\bm \mu}[N_k(T)]\leq  \color{blue}\frac{1}{d(\mu_k,\mu_\star)}\color{black}{\ln T} + \mathcal{O}(\sqrt{\ln(T)}).\]}
...
%     \[A_{t+1} = \argmax_{a = 1,\dots,K} \ \red \widehat{\mu}_a(t) + \sqrt{\frac{\ln_+\left(\frac{T}{KN_a(t)}\right)}{N_a(t)}}\]
...
%                       \[\bE_{\nu}[N_a(T)] \leq \blue\frac{\ln(T)}{\kl(\mu_a,\mu_\star)}\black + \mathcal{O}\left(\ln^{2/3}(T)\right)\]
...
%             \begin{eqnarray*}
%                 \UCB_a^{\text{KL}}(t)& = & \max \left\{ q : N_a(t) \times \kl\left(\widehat{\mu}_a(t), q\right) \leq \ln_+\left(\blue\frac{T}{KN_a(t)}\black\right) \right\}\footnote{$\kl(x,y)=\kl_{\text{Ber}}(x,y)$; \text{can also rely on the non-parameteric KL-UCB index}},\\
%                 \UCB_a^{\text{M}}(t)& = &\widehat{\mu}_a(t) + \sqrt{\frac{\ln_+\left(\blue\frac{T}{KN_a(t)}\black\right)}{2N_a(t)}}.
...
%     \end{itemize}
%     \[G(\pi_a(t-1),n) \simeq \max \left\{ q :  N_a(t)\times \kl\left(\widehat{\mu}_a(t),q\right) \leq \ln \left(\color{red}\frac{n}{N_a(t)}\color{black}\right)\right\}\]
...
%     \end{itemize}
%     \[I_a(t)=\max \left\{ q : N_a(t) \times \kl\left(\widehat{\mu}_a(t),q\right) \leq \ln\left(\color{red}\frac{T}{N_a(t)}\color{black}\right)\right\}\]
%     \begin{itemize}
...
\begin{frameO}[Beyond Bernoulli bandits]
    ...
    \begin{itemize}
        \item For \red one-dimensional exponential families \black, Bayes-UCB rewrites
              \[
                  A_{t+1} = \underset{a}{\text{argmax}} \  Q\left(1-\frac{1}{t(\ln t )^c}, \pi_{a,N_a(t),\widehat{\mu}_a(t)}\right)
              \]
    \end{itemize}
    ...
    \begin{orangeblock}{Theorem \gray [K. 17]}
        ...
        \[\red\bE_{\bm \mu}[N_a(T)] \leq \frac{1+\varepsilon}{\kl(\mu_a,\mu_\star)}\ln(T) + \mathcal{O}_{\varepsilon}(\sqrt{\ln(T)})\black.\]
...
\begin{frameO}[An interesting by-product]
    ...
    \begin{orangeblock}{\klUCB-H$^+$}
        \[u_a^{H,+}(t)=\max\left\{q :  N_a(t)\times \kl\left(\widehat{\mu}_a(t),q\right) \leq \ln\left(\blue\frac{T\ln^c T}{N_a(t)}\black\right)\right\}\]
    \end{orangeblock}

    \begin{orangeblock}{\klUCB$^+$}
        \[u_a^{+}(t)=\max\left\{q  :  N_a(t) \times \kl\left(\widehat{\mu}_a(t),q\right) \leq \ln\left(\blue\frac{t\ln^c t}{N_a(t)}\black\right)\right\}\]
    \end{orangeblock}
...
\begin{frameO}[Thompson Sampling is asymptotically optimal]
    ...
    \begin{orangeblock}{Problem-dependent regret}
        \[\forall \varepsilon>0, \ \ \bE_{\bm\mu}[N_a(T)] \leq \frac{\color{red}1+\varepsilon\color{black}}{\color{red}\kl(\mu_a,\mu_\star)\color{black}}\ln(T) + o_{\mu,\varepsilon}(\ln(T)).\]
...
\begin{frameO}[Where to know more? \hfill{} ($2/3$)]
    ...
    Reach out by email if you have questions
