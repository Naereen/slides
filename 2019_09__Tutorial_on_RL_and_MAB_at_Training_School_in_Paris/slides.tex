...
\begin{frameO}[The Worst-case Performance of UCB]
    ...
    \end{eqnarray*}
    \begin{itemize}
        \item[$\rightarrow$] not exactly matching the $\blue\Omega(\sqrt{KT})$ lower bound...
        but close!
    \end{itemize}
...
%     \begin{itemize}
%         \item[$\rightarrow$] not good enough...
%     \end{itemize}
...
%     \begin{itemize}
%         \item[$\rightarrow$] looks like the \blue \klUCB{} index\black, with a \blue different exploration rate\black...
%     \end{itemize}
...
\subsection{Remove stationary hypothesis: non-stationary bandits}

\begin{frameO}[Remove stationary hypothesis: non-stationary bandits]
...
\subsection{Take more than one action at a time: combinatorial bandits}

\begin{frameO}[Take more than one action at a time: combinatorial bandits]
...
\begin{frameO}[TS for \emph{structured} stochastic bandits ?]

  \begin{itemize}
    \item Arms are edges on a graph
    \item $\mathcal{M}$ is a set of possible configurations (subsets of edges)
    \item The agent chooses $m_t \in \mathcal{M}$ at time $t$ and observe a realization of all arms in $\mathcal{M}$
  ...
  \textbf{Example}: find a matching between users and channels


  \hfill \color{gray} Lelarge et al., \emph{Spectrum Bandit Optimization}, ITW 2013

\end{frameO}


\subsection{More than one player at a time: multi-player bandits}

\begin{frameO}[More than one player at a time: multi-player bandits]

    FIXME

\end{frameO}

\section{Beyond the stochastic MAB}
...
%   \begin{itemize}
%     \item[$\rightarrow$] may be a bad idea...
%   \end{itemize}
...
\begin{frame}{Multi-players bandits: setup}

  \orange $M$ players \black playing \textit{the same} $K$-armed bandit \ \ ($M \leq K$)

  \vspace{0.3cm}

  At round $t$,
  \begin{itemize}
    \item player $m$ selects $A_{m,t}$
    \item player $m$ \textit{observes} $X_{A_{m,t},t}$
  ...
  \begin{itemize}
    \item maximize $\sum_{m=1}^M \sum_{t=1}^T X_{m,t}$
    \item ... \orange without communication \black between players
  \end{itemize}
...
\begin{frame}{Multi-players bandits: algorithms}
  ...
  \alt<2>{\textbf{Remarks}:
    ...
    \textbf{Early references}:


    \begin{itemize}
      \item[] \color{gray}Liu and Zhao, \emph{Distributed Learning in Multi-Armed Bandit with
              Multiple Players},  IEEE Trans. S. P., 2010
      \item[] \color{gray}Anandkumar et al., \emph{Distributed Algorithms for Learning and
              Cognitive Medium Access with Logarithmic Regret}, IEEE Journal on Selected Areas in Communications, 2011
    \end{itemize}}
...
\subsection{Extension: contextual bandits}

\begin{frameO}[Extension: contextual bandits]
...
\subsection{Non discrete actions: continuous-armed bandits}

\begin{frameO}[Non discrete actions: continuous-armed bandits]
...
\subsection{Extension: delayed feedback bandits}

\begin{frameO}[Extension: delayed feedback bandits]
...
\subsection{Extension: cascading bandits}

\begin{frameO}[Extension: cascading bandits]
...
\subsection{Extension: structured bandits (low-rank etc)}

\begin{frameO}[Extension: structured bandits (low-rank etc)]
...
\subsection{Extension: bandits with states, (rested/restless) Markov models}

\begin{frameO}[Extension: bandits with states, (rested/restless) Markov models]
