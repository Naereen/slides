...
author:
  \textbf{Lilian Besson} \newline
  \and R.Bonnefoi \and É.Kaufmann \and C.Moy \and J.Palicot
...
date: 23 Nov 2017 - SCEE Seminar
smalldate: 23/11/17 SCEE Seminar
lang: english
babel-lang: english
handout: false
numbersections: true
...
# Initial motivation : radio problem
...
\begin{figure}[h!]
\centering
\includegraphics[height=0.74\textheight]{crowncom/50intelligent.eps}
...
\begin{figure}[h!]
\centering
\includegraphics[height=0.74\textheight]{crowncom/100intelligent.eps}
...
## Easier model
- "Easy" case : $M \leq K$ devices, always communicating.
- Still interesting: many mathematical and experimental results!

## Two variants
- *With sensing*: Model the "classical" Opportunistic Spectrum Access problem (Wassim's and Navik's theses, 2012, 2017). Not exactly suited for IoT but it can be analyzed mathematically...
- *Without sensing*: like our IoT model but smaller scale. Still very hard to analyze.
...
## Notation
- $K$ channels, modeled as Bernoulli ($0/1$) distributions
  $=$ background traffic from Primary Users
- $M$ devices play $A^j(t) \in \{1,\dots,K\}$ at each time step,
- Reward: $r^j(t) := Y_{A^j(t),t} \indic(\overline{C^j(t)})$
  with $Y_{k,t}$ is the sensing information,
  $C^j(t)$ the collision event for device $j$.
...
- Not the network throughput
- Now we study the centralized regret
  $$R_T := \mathbb{E}\left[ \sum blabla \right] $$

## Goals
- $R_T = \mathcal{O}(T)$ but we want a sub-linear regret
...
- *Is this algorithm efficient in this setting?* \newline
  $\hookrightarrow$ **Upper Bound** on regret for *one* algorithm !

----

# Asymptotic Lower Bound on regrets

- Include plot to illustrate the lower bounds (for rhoRand UCB for instance).
- Talk about the previous state of the art results

----

# Asymptotic Lower Bound on regrets

- Regret is decomposed in $3$ terms
- The term for sub-optimal arms selections is lower-bounded,
  using technical information theory elements (Kullback-Leibler divergence, entropy)

## Theorem
For any decentralized learning algorithm satisfying the strong uniform efficiency,
$R_T \leq \log(T)$ blabla

----

# Algorithms for this easier model

- We need simple (low-cost, low-memory) algorithms
- Goal : 
