...
\begin{frameO}[We implemented this with real hardware \hfill{} ($1/3$)]
    ...
    % \hfill{}
    \begin{small}
        \textcolor{gray}{Ref: [Bonnefoi et al, ICT 2018], [Besson et al, WCNC 2019], Ch.5.3}
        \newline
...
\begin{frameO}[Regret for multi-player bandits ($M$ players for $K$ arms)]
    ...
    \begin{colorblock}{Regret decomposition (if all players $M$ follow $\cA$) \hfill{} {\small \textcolor{gray}{[Besson \& Kaufmann, 18]}}}
...
\begin{frameO}[The MC-Top-$M$ algorithm (for the OSA case)]
    ...
    \begin{small}
        ...
        \begin{itemize}
            \item length of sequence of transitions \textcolor{blue}{$(2)$} is constant
...
\begin{frameO}[Theoretical results for MC-Top-$M$]
    ...
    \only<1>{
        ...
        \hfill{} \textcolor{gray}{Ref: [Cappé et al. 13]}
        \vspace{-0.3cm}
    ...
    \only<2>{
        \begin{center}
            $\bE\left[\sum\limits_{k=1}^K \textcolor{red}{\cC_k(T)}\right] \leq \mathcal{O}(\textcolor{red}{M^2} \textcolor{blue}{\mathbf{C_{M,\mu}}} \log T)$
        \end{center}
    }
    \end{colorblock}
...
\begin{frameO}[Piece-wise stationary bandits]
    ...
    \begin{blueblock}{Stationary MAB problems}
        Arm $k$ \slotmachine{} samples rewards from \textcolor{blue}{the same distribution} for any round
