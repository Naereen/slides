...
\begin{frameO}[Main questions]
    ...
    \begin{colorblock}{Main questions\dots?}
        \begin{itemize}
            \item
            Can the IoT devices optimize their access to the radio resources\\
...
\begin{frameO}[Transition: from practice to theory]
    ...
    \begin{colorblock}{}
        ...
        \begin{itemize}
            ...
            \item \underline{Model 2:} \textbf{non-stationary bandits} (for one device)
        \end{itemize}
        \hfill{}
...
\begin{frameO}[Theoretical results]

    \vspace{0.2cm}
    MC-Top-$M$ with $\kl$-based confidence intervals \hfill{} \textcolor{gray}{[Cappé et al. 13]}
    \vspace{-0.3cm}
    \[\UCB^m_k(t) = \max \left\{ q : N^m_k(t) \kl \left(\hat{\mu}_k^m(t),q\right) \leq \ln(t) \right\},\]
    where $\kl(x,y) = \KL\left(\cB(x),\cB(y)\right) = x \ln \frac{x}{y} + (1-x)\ln \frac{1-x}{1-y}$.

    \begin{darkblock}{Control of the \textcolor{orange}{sub-optimal selections}}
    \only<1>{
        For all sub-optimal arms $k \in \{M+1,\dots,K\}$,
        \vspace{-0.3cm}
        \[
            \bE[N_k^m(T)] \leq \alt<2>{\purple\frac{\ln(T)}{\kl(\mu_k,\mu_{M})}\bluedark}{\frac{\ln(T)}{\kl(\mu_k,\mu_{M})}}+ C_{\boldsymbol{\mu}} \sqrt{\ln(T)}.
        \]
        \vspace{-0.2cm}
    }
    \end{darkblock}

    \begin{colorblock}{Control of the \textcolor{orange}{collisions}}
    \only<1>{
        \[
            \bE\left[\sum_{k=1}^K \cC_k(T)\right]
    ...
    \only<2>{
    \begin{lightblock}{$\implies$ \textbf{logarithmic regret!}}
        \[
            R_{\bm\mu}(\cA,T) = \textcolor{red}{M^2} \textcolor{blue}{\mathbf{C_{M,\mu}}} \log(T) + o(\log(T)),
        \]
...
\begin{frameO}[Results on a multi-player MAB problem ($1/2$)]
    ...
    \begin{figure}
        ...
        \footnotesize{For $K=M$ objects, \textcolor{lightgold}{our strategy MC-Top-$M$} achieves \textbf{constant} regret!\newline \textcolor{lightgold}{MC-Top-$M$} orthogonalization procedure is efficient!}
    \end{figure}
...
\begin{frameO}[Results on a multi-player MAB problem ($2/2$)]
    ...
    \begin{figure}
        ...
        \small{For example for $M=6$ objects, \textcolor{lightgold}{our strategy MC-Top-$M$} largely outperforms \textcolor{lightblue}{$\rho^{\text{rand}}$} and other previous state-of-the-art policies (not included). ({\small log-log scale})}
    \end{figure}
...
\begin{frameO}[Our new algorithm: klUCB index + BGLR detector]
    ...
    \only<1>{
    \begin{colorblock}{Three components of our algorithm \hfill{} \gray [Besson \& Kaufmann, 2019]}
        \begin{itemize}
...
\begin{frameO}[Results on a piece-wise stationary MAB problem]
    ...
    \end{center}
    \vspace*{-15pt}
    $\hookrightarrow$ \textcolor{deeppurple}{klUCB + BGLR ($\star$)} achieves the best performance (among non-oracle)!
\end{frameO}
...
\begin{frameO}[Conclusion: contributions \hfill{} ($1/4$)]
...
\begin{frameO}[Perspectives \hfill{} ($4/4$)]
    ...
    % \begin{lightblock}{What could be done with another year(s) ?}
        ...
        \begin{itemize}[<+->]
            ...
            (cascading, delay feedback, combinatorial, contextual etc)\newline
                \hfill{} {\small $\hookrightarrow$ anyone can contribute on \href{https://GitHub.com/SMPyBandits/SMPyBandits/}{\textcolor{blue}{\texttt{GitHub.com/SMPyBandits/SMPyBandits}}}}
        \end{itemize}
    % \end{lightblock}
