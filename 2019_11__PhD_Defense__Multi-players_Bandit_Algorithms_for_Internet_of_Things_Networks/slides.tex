...
\begin{frameTTnobottom}
    ...
    \begin{textblock*}{12cm}(13mm,50mm)
        ...
        %
        Seminar of the \textbf{PANAMA team}\\
        at \textbf{INRIA} Rennes Bretagne Atlantique\\
        Wednesday $13$th of November, $2019$
        % %
        % \textbf{PhD defense}\\
        % at \textbf{CentraleSupélec}, campus de Rennes\\
...
\begin{frameO}[Internet of Things]

    % Test !

    \begin{columns}%[onlytextwidth]
        \begin{column}{0.65\textwidth}
            \begin{colorblock}{Main hypotheses}
                % \small
                \begin{itemize}
    ...
    \vfill{}
    {\tiny \textcolor{gray}{From \texttt{YouTu.be/QSIPNhOiMoE} and
        \texttt{IBM.com/blogs/internet-of-things/what-is-the-iot}
    }}
...
\begin{frameTI}
    \begin{center}
        \color{white} \Huge \textsc{Outline of this presentation}
...
\begin{frameO}[Outline of this presentation]
    ...
    \begin{itemize}
        ...
        % \end{itemize}
        \item \textbf{Part II.} \emph{Two tractable models}, extending the classical bandit model
        \begin{itemize}
...
\begin{frameO}[GNU Radio for the UI of the demo ($3/3$)]
    \begin{center}
        ...
        \movie{\includegraphics[width=1.05\textwidth]{UI}}{videos/Partie2.mp4}
    \end{center}
\end{frameO}
...
\begin{frameO}[Transition: from practice to theory]
    ...
    \begin{itemize}
        ...
        \item
            But \alert{random activation times} make the model \textbf{hard to analyze}\dots
        \item \underline{Hyp 1:} in expectation, there are $p \times D$ dynamic devices using $K$ channels
            \hfill{} $\implies$ so $p \leq \frac{K}{D}$ or $D \leq \frac{K}{p}$ gives best performance
        \item \underline{Hyp 2:} we assumed a stationary background traffic\dots
    \end{itemize}
    ...
    \begin{colorblock}{}
        ...
        \begin{itemize}
            \item \underline{Model 1:} \textbf{multi-player bandits}: devices \includegraphics[height=0.37cm]{dynamic-devices.png} are always activated\\
                ie. $p=1$ in their random activation process $\implies$ $D = M \leq \frac{K}{p} = K$
            \item \underline{Model 2:} \textbf{non-stationary bandits}
        \end{itemize}
        \hfill{}
...
\begin{frameO}[M-P bandits for Cognitive Radios: proposed models]
    ...
    \begin{colorblock}{\# 1: ``Talk and maybe collide''
    ...
    \begin{lightblock}{\# 1: ``Observe collision then talk?''
        \hfill{} {\tiny \textcolor{lightgray}{[Besson \& Kaufmann, ALT 2018], [Boursier et al, 2019]}}
    }
        \begin{itemize}
            \item
            A third ``hybrid'' model
            {\small studied by 4 papers in 2019, following our work}
...
\begin{frameO}[Regret for multi-player bandits ($M$ players for $K$ arms)]
    ...
    \begin{colorblock}{Regret decomposition (if all players $M$ follow $\cA$)}
...
\begin{frameO}[Our new algorithm: klUCB index + BGLR detector]
...
\begin{frame}[shrink=17]
    ...
    \begin{itemize}
        ...
        \item ``\emph{GLRT meets klUCB \dots}'', \textbf{Besson} \& Kaufmann \& Maillard, for \textcolor{darkred}{AISTATS}, \textcolor{blue}{October 2019}
