...
\newcommand{\tgvsmall}{\includegraphics[height=0.23cm]{high-speed-train_1f684.png}}
...
\begin{frameO}[Who am I ?]
    ...
    \begin{itemize}
        \item finishing my PhD, in Telecommunications and Machine Learning
...
\begin{frameO}[But\dots world-wide non homogeneous frequency usage]
    ...
    \begin{center}
        \includegraphics[width=0.80\textwidth]{scan_WiFi^maison}
    \end{center}
    \hfill{}
...
\begin{frameO}[What did I study during my PhD ? ($1/2$)]
    ...
    \begin{center}
        % \centering
        \includegraphics[width=0.50\linewidth]{system^model1.eps}
...
\begin{frameO}[Reading map of my thesis for today]
    ...
    \begin{figure}[h!]
        ...
        \begin{tikzpicture}[>=latex',line join=bevel,scale=2.25]
            ...
            \node[align=center] (chapter2) at (0,2.25) [rectangle,draw,fill=gray!10,text=gray] {Chapter~2\\The Stochastic\\Multi-Armed Bandit models};
...
\begin{frameO}[Outline of this talk]
    ...
    \begin{itemize}
        ...
        % \end{itemize}
        \item Selfish MAB learning in a new model of IoT network
        \begin{itemize}
        ...
        \end{itemize}
        \item \emph{Two tractable models}, extending the classical bandit model
        \begin{itemize}
...
\begin{frameO}[We implemented this with real hardware ($1/3$)]
    ...
    \end{center}
    \hfill{}
    {\footnotesize \textcolor{gray}{Ref: [Bonnefoi et al, ICT 2018], [Besson et al, WCNC 2019], Ch.5.3}}
\end{frameO}


\begin{frameO}[Using USRP board to simulate IoT objects ($2/3$)]
    \begin{center}
...
\begin{frameO}[GNU Radio for the UI of the demo ($3/3$)]
    \begin{center}
...
\begin{frameO}[Transition: from practice to theory]
    ...
    \begin{itemize}
        ...
        \item
            In expectation, there are $p \times D$ dynamic devices using $K$ channels\\
            $\implies$ so $p \leq \frac{K}{D}$ or $D \leq \frac{K}{p}$ gives best performance
        \item
            We assume a stationary hypothesis on the background traffic\dots
        \item
            The random activation times make the model \textbf{intractable} in theory\dots
    \end{itemize}
    ...
    \begin{colorblock}{}
        ...
        \begin{itemize}
            \item \textbf{multi-player bandits}: devices are always activated\\
                ie. $p=1$ in their random activation process $\implies$ $D = M \leq \frac{K}{p} = K$
            \item \textbf{non-stationary bandits}
        \end{itemize}
        \hfill{}
        $\implies$ Both are \textbf{tractable models} !
    \end{colorblock}
...
\begin{frameO}[Multi-players bandits: setup]
    ...
    \orange $M \geq 2$ players \black playing \textit{the same} $K$-armed bandit
    \hfill{} ($2 \leq M \leq K$)
    \newline
    ...
    \begin{itemize}
        \item player $m$ selects arm $A^m_{t}$ \slotmachine{} ; then \textit{this arm generates} $s_{A^m_{t},t}$
    ...
    \[\red r_{m,t} = \left\{ \begin{array}{cl}
            r_{A^m_{t},t} & \text{if no other player chose the same arm} \\
...
\begin{frameO}[M-P bandits for Cognitive Radios: previous model]
    ...
    Different observation models: players observe $s_{A^m_{t},t}$ and/or $r_{m,t}$

    \begin{darkblock}{``Listen before talk''
...
\begin{frameO}[M-P bandits for Cognitive Radios: proposed models]
    ...
    \begin{colorblock}{``Talk and maybe collide''
        ...
        \begin{itemize}
            ...
            \item
            \emph{Feedback model:} observe only the joint information $r_{m, t}$.\\
    ...
    \begin{lightblock}{``Observe collision then talk?''
        \hfill{} \textcolor{lightgray}{[Boursier et al, 2019]}
    }
        \begin{itemize}
            \item
            A third ``hybrid'' model, studied by 4 papers following our work

            \item
            \emph{Feedback model:} First check if collision, then receive reward $r_{m, t}$\newline
...
\begin{frameO}[Regret in the homogeneous case]
    ...
    \only<2-2>{\vspace{-0.2cm}}
     ...
     Arms sorted by decreasing utility: $\mu_1 \geq \mu_2 \geq \dots \geq \mu_K$
      $$R_{\bm\mu}(\cA,T)
      :=
      \underbrace{\left(\sum_{k=1}^{M}{\mu_k}\right) T}_{\text{oracle total reward}} - {\bE_{\bm\mu}^{\cA}}\left[\sum_{t=1}^T\sum_{m=1}^M r_{m,t} \right]$$
    ...
    \alt<2>{\vspace{0.2cm}
    \[R_{\bm\mu}(\cA,T) \leq C  \sum_{k=M+1}^K \bE\left[\textcolor{orange}{N_k(T)}\right] + D\sum_{k=1}^M \bE\left[\textcolor{red}{\mathcal{C}_k(T)}\right].\]
    ...
    \alt<2>{We need to control:
    \begin{itemize}
     \item the number of \textcolor{orange}{selections of sub-optimal arms}\newline
        $\hookrightarrow$ with a good classical bandit policy: like klUCB
     \item the number of \textcolor{red}{collisions} on optimal arms\newline
        $\hookrightarrow$ with a good orthogonalization procedure
    \end{itemize}}{
    \begin{itemize}
     \item $N_k(T)$ total number of \textcolor{orange}{selections} of arm $k$
     \item $\cC_k(T)$ total number of \textcolor{red}{collisions} experienced on arm $k$
    \end{itemize}}
...
\begin{frameO}[The MC-Top-M algorithm]
    ...
    \underline{Other UCB-based algorithms}:

    \vspace{0.2cm}

    TDFS \color{gray} [Lui and Zhao 2010]\black, Rho-Rand \color{gray} [Anandkumar et al. 2011] \black
...
\begin{frameO}[The MC-Top-M algorithm]
    ...
    Two simple ideas:
    {\small $\hookrightarrow$ inspired by Musical Chair \color{gray}[Rosenski et al. 2016] \color{black}}
    \begin{itemize}
        \item always pick $\color{red} A^m(t) \in \hat{M}^{m}(t-1)$
        \item try not to switch arm too often
    \end{itemize}
    \[\orange s^m(t) = \left\{ \text{player } m \text{ is ``fixed'' at the end of round } t \right\}\]
    ...
    \begin{itemize}
     \item if $A^m(t-1) \notin  \hat{M}^{m}(t-1)$, \\ set $\orange s^m(t) = \mathrm{False}$ and \color{orange}carefully select a new arm in $\hat M^m(t-1)$\color{black}.
     \item else if $\overline{s^m(t-1)}\cap C^m(t-1)$, pick a new arm at random

     \vspace{-0.2cm}

      \[A^m(t) \sim \mathcal{U} (\hat{M}^m(t-1)) \ \ \text{and} \ \ s^m(t) = \mathrm{False} \]
     \item else, draw the previous arm, and fix on it

     \vspace{-0.2cm}

      \[A^m(t) =A^m(t-1) \ \ \text{and} \ \ \orange s^m(t) = \mathrm{True}\]
    \end{itemize}
...
\begin{frameO}[Results on a multi-player MAB problem ($1/2$)]
    ...
    \begin{figure}
        ...
        \footnotesize{For $K=M$ objects, our strategy MC-Top-$M$ achieves \textbf{constant} regret!\newline MC-Top-$M$ orthogonalization procedure is efficient!}
    \end{figure}
...
\begin{frameO}[Results on a multi-player MAB problem ($2/2$)]
    ...
    \begin{figure}
        ...
        \small{For example for $M=6$ objects, our strategy MC-Top-$M$ largely outperforms $\rho^{\text{rand}}$ and other previous state-of-the-art policies (not included).}
    \end{figure}
...
\begin{frame}[shrink=22]
    ...
    \begin{center}
        \begin{tabular}{ m{2.7cm} | c | c | c | c | m{3cm} }
            ...
            & \parbox[c]{1.5cm}{\textbf{\small Complexity} \newline {\footnotesize \textcolor{gray}{\tgvsmall{} is worst}}}
            & \textbf{Parameters} \\
            \hline
            \textcolor{deeppurple}{\small Centralized multi-play klUCB} & \textcolor{gray}{[1]} & $\textcolor{blue}{\mathbf{C_{M,\mu}}} \log(T)$ & {\textcolor{orange}{\etoile{}\etoile{}\etoile{}\etoile{}\etoile{}}} & {\textcolor{orange}{\tgv{}\tgv{}}} & {just $M$ but \newline \textcolor{deeppurple}{in another model!} } \\
            % \vspace*{3pt}
            \hline
            % \vspace*{3pt}
            ...
            \hline
            % \vspace*{3pt}
            \textcolor{orange}{MCTopM klUCB} & \textcolor{orange}{[6]} & $\textcolor{red}{M^2} \textcolor{blue}{\mathbf{C_{M,\mu}}} \log(T)$ & {\textcolor{orange}{\etoile{}\etoile{}\etoile{}\etoile{}}} & {\textcolor{orange}{\tgv{}\tgv{}}} & just $M$ \\
            \hline
    ...
    \begin{small}
        \textbf{Papers:}
            [1] \textcolor{gray}{Anantharam, 87}
            [2] \textcolor{gray}{Anandkumar, 11}
            [3] \textcolor{gray}{Avner, 15}
            [4] \textcolor{gray}{Rosenski, 15}
            \newline\hspace*{10pt}
            [5] \textcolor{orange}{Bonnefoi et al. 17}
            [6] \textcolor{orange}{Besson \& Kaufmann, 18}
            [7] \textcolor{gray}{Boursier et al. 19}
            [8] \textcolor{gray}{Proutière, 19}
    \end{small}
...
\begin{frame}[c]
    \begin{changemargin}{-0.5cm}{-0.5cm}
        \begin{center}
            ...
            \small{\textcolor{gray}{Ref: Chapter 7 of my thesis, and [Besson \& Kaufmann, 2019].}}
        \end{center}
    \end{changemargin}
\end{frame}
...
\begin{frameO}[Regret for piece-wise stationary bandits]
    ...
    \begin{colorblock}{Typical regime for piece-wise stationary bandits}
        \begin{itemize}
            \item ``Oracle restart'' UCB obtains
                $\cR(\cA,T) \leq \mathcal{O}((K / \Delta_{\text{opt}}^2) \Upsilon_T \log(T))$

            \item The (minimax) lower-bound is
...
\begin{frameO}[Our proposed algorithm \textcolor{blue}{klUCB index + BGLR detector}]

    \begin{alertblock}{}
        Our algorithm is the new state-of-the-art!

        Inspired by CUSUM-UCB
        \textcolor{gray}{[Liu et al. 18]}
        and M-UCB
        \textcolor{gray}{[Cao et al. 19]}
    \end{alertblock}

    \begin{colorblock}<1>{Three parts of the algorithm \hfill{} \gray [Besson \& Kaufmann, 2019]}
        \begin{itemize}
            \item A classical bandit index policy: \textbf{klUCB}\newline
                which gets \emph{restarted} after a change-point is detected
            \item A change-point detection algorithm:
            the \textbf{Generalized Likelihood Ratio Test} for sub-Bernoulli observations (BGLR),
            we can bound
            \begin{itemize}
                \item its false alarm probability (if enough samples between two restart)
                \item its detection delay (under some hypotheses of ``easy enough'' problems)
            \end{itemize}
            \item Forced exploration of parameter $\alpha$ (tuned with knowledge of $\Upsilon_T$)
        \end{itemize}
    \end{colorblock}

    \begin{lightblock}<2>{Theoretical guarantees}
        Our algorithm $\cA$ obtains
        \begin{itemize}
        % \tightlist
            \item
            ...
            \item
                ...
                if $T$ and \textcolor{orange}{$\Upsilon_T$ are unknown}
        \end{itemize}
        with precise constants in the $\mathcal{O}(\dots)$ depending on the problem difficulty\dots
    \end{lightblock}
...
\begin{frameO}[Results on a piece-wise stationary MAB problem]
    ...
    \end{center}
    \vspace*{-15pt}
    $\hookrightarrow$ \textcolor{blue}{klUCB + BGLR} achieves the best performance (among non-oracle)!
\end{frameO}
...
\begin{frame}[shrink=23]
    ...
    \begin{tabular}{ m{3.5cm} | c | c | c | c | m{3cm} }
        ...
        \hline
        AdSwitch & \textcolor{gray}{[8]} & $\textcolor{red}{\mathbf{C}_8} \sqrt{T \Upsilon_T \log(T)}$ & {\textcolor{orange}{\etoile{}\etoile{}\etoile{}\etoile{}}} & {\textcolor{orange}{\tgv{}}} & just $T$ \\
        % \hline
...
\begin{frameO}[Conclusion: what we showed \hfill{} ($1/4$)]
    ...
    % \begin{colorblock}{What we showed}
        ...
        \item New algorithms and regret bounds, in two simplified models:
            ...
            \item our proposed algorithms achieve state-of-the-art performance on both numerical and theoretical results.
        \end{itemize}
    % \end{colorblock}
...
\begin{frame}[shrink=17]
    \mytitle{centralesupelecdark}{centralesupelec}{List of publications \hfill{} ($3/4$)}
...
\begin{frameO}[What could be done with another year(s) ? \hfill{} ($4/4$)]
    ...
    % \begin{lightblock}{What could be done with another year(s) ?}
        ...
        \begin{itemize}[<+->]
            ...
            (cascading, delay feedback, combinatorial, contextual etc)\newline
                \hfill{} {\small $\hookrightarrow$ you can contribute on \href{https://GitHub.com/SMPyBandits/SMPyBandits/}{\textcolor{blue}{\texttt{GitHub.com/SMPyBandits/SMPyBandits}}}}
        \end{itemize}
    % \end{lightblock}
...
\begin{frameO}[Backup slides to answer predicatable questions ?]

    \begin{itemize}
        \item Add pseudo code of MCTopM + klUCB
        \item Add exact regret bound of MCTopM + klUCB
        \item Add drawing explaining the proof of regret bound of MCTopM + klUCB
    \end{itemize}

    \begin{itemize}
        \item Add pseudo code of GLRT + klUCB
        \item Add exact regret bound of GLRT + klUCB
    \end{itemize}
