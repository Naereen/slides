...
\begin{frameTTnobottom}
    ...
    \begin{textblock*}{12cm}(13mm,50mm)
        ...
        \\
        {\large
        Seminar of the SCEE team\\
        at CentraleSupélec, campus de Rennes\\
...
\begin{frameO}[Who am I ?]
    ...
    \begin{itemize}
        ...
        \item under supervision of Prof. Christophe Moy at IETR \& CentraleSupélec in Rennes (France)
        \item and Dr. Émilie Kaufmann in Inria in Lille\\
        \hfill{} \emph{Thanks to Émilie Kaufmann for some of the slides material!}
    \end{itemize}

    \vspace*{5pt}

    \begin{itemize}
        \item \href{https://perso.crans.org/besson/}{\textcolor{darkgreen}{\texttt{Lilian.Besson @ CentraleSupelec.fr}}}

        \item $\hookrightarrow$ \href{https://perso.crans.org/besson/}{{\textcolor{darkgreen}{\texttt{perso.crans.org/besson/}}}} \& \href{https://GitHub.com/Naereen/}{{\textcolor{darkgreen}{\texttt{GitHub.com/Naereen}}}}
    \end{itemize}
...
\begin{frameO}[What did I study during my PhD ? ($1/2$)]
    ...
    \begin{alertblock}{Telecommunications technology\dots}
    ...
    \begin{colorblock}{Main questions}
        \begin{itemize}
            \item
            Can the devices optimize their access to the radio resources\\
            in a \emph{simple}, \emph{efficient}, \emph{automatic} and \emph{decentralized} way?

            \item
            % $\hookrightarrow$
            \alert{Can the devices learn on their own to communicate more efficiently?}
        \end{itemize}
    \end{colorblock}
...
\begin{frameO}[What did I study during my PhD ? ($2/2$)]
    ...
    \begin{alertblock}{Main solutions}
        Yes! \\
        ...
        \hspace{15pt} $\hookrightarrow$ we focussed on \alert{Multi-Armed Bandit} algorithms\\
        \hspace{30pt} and we showed they are well suited for these problems!
    \end{alertblock}
...
\begin{frameO}[Main contributions presented today]
    ...
    \begin{itemize}
        ...
        \item proofs in a simplified model: multi-player bandit,
...
\begin{frameO}[Chapters of my thesis addressed today]
    ...
    \begin{enumerate}[leftmargin=50pt]
        ...
        \item[Ch.3)] \textcolor{gray}{SMPyBandits: an exhaustive Python library to simulate MAB problems}
        \item[Ch.4)] \textcolor{gray}{Expert aggregation for online MAB algorithms selection}
...
\begin{frameO}[Organization of the thesis: a reading map]
...
\begin{frameO}[Outline of this talk]
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item Performance measure (regret) and first strategies
            \item Best possible regret? Lower bounds
            \item Upper Confidence Bounds (UCB) Algorithms
        \end{itemize}
        \item Selfish MAB learning in our model of IoT network
        \begin{itemize}
            \item reference strategies
            \item selfish applications of UCB algorithms
            \item intractable model in theory\dots
        \end{itemize}
        \item Many extensions of the stationary single-player bandit models
        \begin{itemize}
            \item multi-player bandits in stationary settings
            \item single-player bandits in piece-wise stationary settings
        \end{itemize}
...
\begin{frameO}[Make money in a casino?]
    \begin{center}
        \includegraphics[height=5cm]{MABpieuvre}
    \end{center}
    \begin{center}
...
\begin{frameO}[Cognitive radios]
    ...
    \textbf{Opportunistic spectrum access} \gray [Zhao et al. 10] [Anandkumar et al. 11]\black
...
% \begin{frameO}[A better idea: Explore-Then-Commit (\texttt{ETC})]
...
%             \item compute the empirical best arm $\widehat{a} = \text{argmax}_{a} \ \widehat{\mu}_a(Km)$
%             \item keep playing this arm until round $T$

%                   \vspace{-0.4cm}

%                   \[A_{t+1} = \widehat{a} \ \ \text{for} \ t \geq Km\]
%         \end{itemize}
%         \color{red}$\Longrightarrow$ EXPLORATION followed by EXPLOITATION\color{black}
%     \end{colorblock}
%     \pause
...
%     \underline{Analysis for $K=2$ arms}. If $\mu_1 > \mu_2$,  $\textcolor{blue}{\Delta := \mu_1 - \mu_2}$.

%     \vspace{-0.5cm}

%     \begin{eqnarray*}\mathcal{R}_{\bm\nu}(\texttt{ETC},T) & = & \Delta \bE[N_2(T)] \\
%         & = & \Delta \bE\left[ m + (T - Km)\ind\left(\widehat{a} = 2\right)\right] \\
%         & \leq &\Delta m + (\Delta T) \times \alt<3>{\red \bP\left(\widehat{\mu}_{2,m} \geq \widehat{\mu}_{1,m}\right)}{\bP\left(\widehat{\mu}_{2,m} \geq \widehat{\mu}_{1,m}\right)}
%     \end{eqnarray*}

%     \vspace{-0.3cm}

%     $\widehat{\mu}_{a,m}$: empirical mean of the first $m$ observations from arm
%     $a$

%     \alt<3>{\hfill{} \red $\Longrightarrow$ requires a concentration inequality}{}
% \end{frameO}
...
%     \hfill{} $\Longrightarrow$ \red Hoeffding's inequality
% \end{frameO}
...
%     \hfill{} $\Longrightarrow$ \red Gaussian tail inequality
% \end{frameO}
...
%     For $\blue m = \frac{4\sigma^2}{\Delta^2}\log\left(\frac{T\Delta^2}{4\sigma^2}\right)$,

%     \vspace{-0.3cm}

%     \[\cR_{\bm\nu}(\texttt{ETC},T) \leq \frac{4\sigma^2}{\Delta}\left[\log\left(\frac{T\Delta^2}{2}\right) + 1\right] = \mathcal{O}\left(\frac{1}{\Delta} \log(T)\right).\]
...
%         \item[$\bm -$] requires the knowledge of $T$ ($\simeq$ OKAY) \red and $\Delta$ \black (NOT OKAY)
%     \end{itemize}
...
%     \[\hspace{-1cm}\blue \tau = \inf \left\{t \in \N : |\widehat{\mu}_1(t) - \widehat{\mu}_2(t) | > \sqrt{\frac{8\sigma^2\log(T/t)}{t}}\right\}  \]
...
%     \begin{itemize}
%         \item $\blue \widehat{a}_\tau = \argmax{}_{a} \ \widehat{\mu}_a(\tau)$ and $(A_{t+1} = \widehat{a}_\tau)$ for $t \in \{\tau+1, \dots, T\}$
%     \end{itemize}

%     \vspace{-0.2cm}

%     \[
%         \cR_{\bm\nu}(\texttt{S-ETC},T)  \leq  \red \frac{4\sigma^2}{\Delta}\log\left(T\Delta^2\right)\black \ + C \sqrt{\log(T)} = \mathcal{O}\left(\frac{1}{\Delta} \log(T)\right).\]

%     \vspace{-0.3cm}

%     \begin{itemize}
%         \item[$\Longrightarrow$] same regret rate, without knowing $\Delta$ \gray [Garivier et al. 2016]
%     \end{itemize}
...
%     \[
%         \cR_{\bm\nu}(\texttt{ETC},T)  \lesssim  \red \frac{4\sigma^2}{\Delta}\log\left(T\Delta^2\right)  = \mathcal{O}\left(\frac{1}{\Delta} \log(T)\right)\black.\]

%     \vspace{-0.3cm}

%     \begin{itemize}
%         \item[$\Longrightarrow$] \alert{problem-dependent logarithmic} regret bound $\cR_{\bm\nu}(\text{algo}, T) = \mathcal{O}(\log(T))$.
%     \end{itemize}

%     \textbf{Observation:} blows up when $\Delta$ tends to zero\dots
%     \begin{eqnarray*}
%         \cR_{\bm\nu}(\texttt{ETC},T)  &\lesssim& \min \left[\frac{4\sigma^2}{\Delta}\log\left(T\Delta^2\right) , \blue\Delta T\black\right] \\
%         & \leq & \sqrt{T} \min_{u > 0} \left[\frac{4\sigma^2}{u}\log(u^2) , u\right]
...
%     \begin{itemize}
%         \item[$\Longrightarrow$] \alert{problem-independent square-root} regret bound $\cR_{\bm\nu}(\text{algo}, T) = \mathcal{O}(\sqrt{T})$.
%     \end{itemize}
...
\section{Best possible regret? Lower bounds}
...
\begin{frameO}[The Lai and Robbins lower bound]
    ...
    \end{colorblock}
    \begin{colorblock}{Theorem \color{gray} [Lai and Robbins, 1985]}
        For uniformly efficient algorithms ($\cR_{\bm\mu}(\cA,T)=o(T^\alpha)$ for all $\alpha\in (0,1)$ and $\bm \mu \in \cI^K$),

        \vspace{-0.4cm}

        \[
            \color{red}\mu_a<\mu_\star \Longrightarrow \liminf_{T\rightarrow\infty}\frac{\bE_{\bm \mu}[N_{a}(T)]}{\log T}\geq \frac{1}{\kl(\mu_a,\mu_\star)}.\color{black}
        \]
...
%               \[
%                   \cR_{\bm\nu}(\mathrm{ETC},T)  \lesssim  \red \frac{4\sigma^2}{\Delta}\black \log\left(T\Delta^2\right) = \mathcal{O}\left(\frac{1}{\Delta} \log(T)\right),\]
...
%               \[
%                   \cR_{\bm\nu}(\cA,T)  \gtrsim  \red \frac{2\sigma^2}{\Delta}\black \log\left(T\Delta^2\right) = \Omega\left(\frac{1}{\Delta} \log(T)\right),\]
%               as $\kl(\mu_1,\mu_2) = \frac{(\mu_1 - \mu_2)^2}{2 \sigma^2}$.
%     \end{itemize}

%     \begin{itemize}
%         \item[$\Longrightarrow$] Explore-Then-Commit is \alert{not} \blue asymptotically optimal\black.
%     \end{itemize}
...
%     \[L_t(\bm\mu,\bm\lambda) := \log \frac{\ell(R_1,\dots,R_t ; \bm\mu)}{\ell(R_1,\dots,R_t ; \bm\lambda)}.\]
...
%     \pause
%     \alt<3>{\[\red \bE_{\bm\mu}[N_a(T)] \gtrsim \frac{\log(T)}{\kl(\mu_a,\mu_\star + \varepsilon)}\]\vspace{-0.4cm}\begin{center}for large values of $T$                                                                                                                                                                                        \end{center}
%     }{
%     \begin{itemize}
%         \item[$\Longrightarrow$]
%               $\sum_{a=1}^K \bE_{\bm \mu}[N_a(T)]\times \kl(\mu_a,\lambda_a) = \red\bE_{\bm \mu}[N_a(T)] \kl(\mu_a,\mu_1+\varepsilon)$ \\
%         \item[$\Longrightarrow$] Picking  $\blue \cE_T = (N_1(T) > T/2)\black$,
%     \end{itemize}
%     \[\red \mathrm{kl}_{\text{Ber}}(\bP_{\bm \mu}(\cE_T),\bP_{\bm \lambda}(\cE_T))\sim \log(T)\]
...
%     \textbf{Context:} a simple \blue parametric bandit model \black $\bm\nu =(\nu_{\mu_1},\dots,\nu_{\mu_K})$, $\mu_a \in \cI$.

%     \begin{colorblock}{Lai and Robbins' lower bound \gray[1985]}
...
%     \begin{itemize}
%         \item[$\Longrightarrow$] can be extended to cover \blue more general classes of bandit instances \black
%     \end{itemize}

%     \begin{colorblock}{Burnetas and Katehakis' lower bound \gray[1996]}
...
%             \liminf_{T\to\infty}\frac{\bE[N_{a}(T)]}{\log T} \geq \frac{1}{\cK_{a}(\nu_a,\mu_\star)},\]
%         where $\cK_{a}(\nu_a,\mu_\star)= \inf \{\KL(\nu_a,\nu) : \nu\in\cD_a,\bE_{X \sim \nu}[X]>\mu_\star\}.$
...
%     \begin{colorblock}{Theorem \gray[Cesa-Bianchi and Lugosi, 06][Bubeck and Cesa-Bianchi, 12]}
...
%     \begin{itemize}
%         \item worst-case model (Bernoulli):
%               \[\left\{\begin{array}{ccl}
%                       \nu_a     & = & \cB(1/2) \ \ \text{for all} \ \ a\neq a_{\star} \\
%                       \nu_{a_{\star}} & = & \cB(1/2+\varepsilon)
%                   \end{array}\right.\]
...
%
% Since the work of Lai and Robbins, some focus has been put on finding algorithms\dots
%
% \vspace{0.3cm}
%
% \begin{itemize}
...
% \section{Mixing Exploration and Exploitation}
...
%     \begin{colorblock}{$\varepsilon_t$-greedy strategy}
%         At round $t$,
%         \begin{itemize}
%             \item with probability $\blue\varepsilon_t :=\min \left(1, \frac{K}{d^2t}\right)$ \hfill{} probability $\searrow$ with $t$
%                   \[A_t \sim \cU(\{1,\dots,K\})\]
...
%                   \[A_t = \argmax_{a = 1,\dots,K} \ \widehat{\mu}_a(t-1).\]
%         \end{itemize}
%     \end{colorblock}

%     \begin{colorblock}{Theorem \gray [Auer et al. 02]\black}
%         If $\red 0 < d \leq \Delta_{\min}$, $\cR_{\bm\nu}\left(\varepsilon_t\texttt{-greedy},T\right) = \mathcal{O}\left(\frac{1}{d^2} K\log(T)\right)$.
%     \end{colorblock}

%     \vspace{-0.3cm}

%     \begin{itemize}
%         \item[$\Longrightarrow$] requires the knowledge of a lower bound on $\Delta_{\min}$.
%     \end{itemize}
...
\section{The Optimism Principle and Upper Confidence Bounds Algorithms}
...
%     \begin{itemize}
%         \item For each arm $a$, build a confidence interval $\cI_a(t)$ on the mean $\mu_a$ :
%               \color{red}
...
%         \[\red\bP\left(\mu_a \leq \mathrm{UCB}_a(t)\right)\gtrsim 1 - 1/t\black.\]
...
%     \hfill{} $\Longrightarrow$ tool: \blue concentration inequalities\black

%     \textbf{Example:} rewards are \red $\sigma^2$ sub-Gaussian\black

%     \vspace{-0.3cm}

%     \begin{equation}\bE[Z] = \mu \ \ \text{and} \ \ \ \bE\left[e^{\lambda(Z-\mu)}\right] \leq e^{\lambda^2\sigma^2 / 2}.\label{SG}\end{equation}

%     \vspace{-0.3cm}

%     \begin{alertblock}{Hoeffding inequality}
%         $Z_i$ i.i.d. satisfying \eqref{SG}. For all (\textbf{fixed}) $s\geq 1$
%         \[\alt<2->{\bP\left(\frac{Z_1 + \dots + Z_s}{s}  \leq \mu - x \right)  \leq e^{-sx^2 / (2\sigma^2)}}{\bP\left(\frac{Z_1 + \dots + Z_s}{s}  \geq \mu + x \right)  \leq e^{- s x^2 / (2\sigma^2)}}\]
%     \end{alertblock}

%     \alt<3>{\color{red}\Large\danger\normalsize\black Cannot be used directly in a bandit model as \blue the number of observations $s$ from each arm is random\black!}{\vspace{-0.3cm}

%         \begin{itemize}
%             \item $\nu_a$ bounded in $[0,1]$: $1/4$ sub-Gaussian
%             \item $\nu_a = \cN(\mu_a,\sigma^2)$: $\sigma^2$ sub-Gaussian
%         \end{itemize}
%     }
...
\begin{frameO}[A first UCB algorithm]
    ...
    \begin{colorblock}{Hoeffding inequality + union bound}
...
\begin{frameO}[A UCB algorithm in action \hfill{} (movie)]
...
\begin{frameO}[Regret of UCB($\alpha$) for bounded rewards]
    ...
    \begin{colorblock}{Theorem \gray[Auer et al, 02]\black}
        ...
        \[\cR_{\bm\nu}(\texttt{UCB1},T) \leq 8 \left(\sum_{a : \mu_a < \mu_\star} \frac{1}{\Delta_a}\right)\log(T) + \left(1+\frac{\pi^2}{3}\right)\left(\sum_{a=1}^K \Delta_a\right).\]
    \end{colorblock}
...
%     \begin{eqnarray*}
%         \sum_{t=0}^{T-1}\bP(\UCB_1(t) \leq \mu_1) & \leq & 1 + \sum_{t=1}^{T-1} \bP\left(\widehat{\mu}_1(t) + \sqrt{\frac{\alpha \log(t)}{N_1(t)}} \leq \mu_1\right) \\
...
%     \[\mathrm{LCB}_a(t) = \widehat{\mu}_{a}(t) - \sqrt{\frac{\alpha \log t}{N_a(t)}}.\]
...
%             \begin{eqnarray*}
%                 &\Longrightarrow &(\mu_1 - \mu_a) \leq 2 \sqrt{\frac{\alpha \log(T)}{N_a(t)}} \\
%                 & \Longrightarrow & \ N_a(t) \leq \frac{4\alpha}{(\mu_1 - \mu_a)^2}\log(T)
%             \end{eqnarray*}
%         \end{column}
%         \begin{column}{4.5cm}
%             \begin{center}
...
%         + C_\alpha /2 \\
%         & \leq &  \sum_{t=0}^{T-1}\bP\left(A_{t+1}=a, N_a(t) \leq \frac{4\alpha}{(\mu_1 - \mu_a)^2}\log(T)\right)+ C_{\alpha}/2 \\
%         & \leq & \frac{4\alpha}{(\mu_1 - \mu_a)^2}\log(T) + C_\alpha/2
%     \end{eqnarray*}
...
%     \[\bE[N_a(T) ] \leq \frac{4\alpha}{(\mu_1 - \mu_a)^2}\log(T) + C_\alpha.\]
...
%     \[\UCB_a(t) = \widehat{\mu}_a(t) + \sqrt{\frac{2\sigma^2(\log(t) + c\log\log(t))}{N_a(t)}}\]

%     \vspace{-0.2cm}

%     \begin{colorblock}{Theorem \color{gray}[Cappé et al.'13]}
%         For $c \geq 3$, the UCB algorithm associated to the above index satisfy
%         \[\bE[N_a(T)] \leq \frac{2\sigma^2}{(\mu_\star - \mu_a)^2}\log(T) + C_{\bm\mu}\sqrt{\log(T)}.\]
%     \end{colorblock}
...
%                   \[\mathcal{R}_{\nu}(\mathrm{UCB},T) \neq\red \left(\sum_{a : \mu_a < \mu_\star} \frac{\Delta_a}{\kl(\mu_a,\mu_\star)}\right)\black \log(T) \]

%                   \vspace{-0.3cm}

%             \item[$\Longrightarrow$] \red not \black matching the Lai and Robbins lower bound
...
%                       \[\mathcal{R}_{\nu}(\mathrm{UCB},T) \lesssim\blue \left(\sum_{a : \mu_a < \mu_\star} \frac{1}{2\Delta_a}\right)\black \log(T) \]

%                       \vspace{-0.3cm}

%                 \item[$\Longrightarrow$] optimal ?
%             \end{itemize}
%         }
%         {\begin{itemize}
...
%                       \[\mathcal{R}_{\nu}(\mathrm{UCB},T) \lesssim \blue \left(\sum_{a : \mu_a < \mu_\star} \frac{2\sigma^2}{\Delta_a}\right)\black \log(T) .\]

%                       \vspace{-0.3cm}

%                 \item[$\Longrightarrow$] matching the Lai and Robbins lower bound! \blue asymptotically optimal \black
%             \end{itemize}}}
...
%     \begin{itemize}
%         \item UCB worst-case regret:  $\mathcal{O}(\blue\sqrt{KT \log(T)})$
%     \end{itemize}
%     \begin{eqnarray*}
%         \cR_{\bm\nu}(\texttt{UCB},T) & = &\sum_{a=1}^K \textcolor{red}{\Delta_a\sqrt{\bE[N_a(T)]}}\sqrt{\bE[N_a(T)]} \\
%         &=&\sum_{a=1}^K \mathcal{O}(\red{\sqrt{\log(T)}}\black)\sqrt{\bE[N_a(T)]}\\
%         &\leq& K\sqrt{\frac{1}{K} \sum_{a}\bE[N_a(T)]}\mathcal{O}(\sqrt{\log(T)})\\
%         &=& \mathcal{O}(\sqrt{KT \log(T)})
%     \end{eqnarray*}
%     \begin{itemize}
%         \item[$\Longrightarrow$] not exactly matching the $\blue\Omega(\sqrt{KT})$ lower bound\dots
%               but close!
%     \end{itemize}
...
%     \[A_{t+1} = \argmax_{a=1,\dots,K} \ \red  \hat\mu_{a}(t) + \sqrt{\frac{2{\widehat{\sigma}_a(t)} \log t^3}{N_a(t)}} + \frac{7\log t^3}{3{N_a(t)}}\]
%     where $\widehat{\sigma}_a(t) =\frac{1}{N_a(t)}\sum_{s=1}^{N_a(t)} \left(Y_{a,s} - \widehat{\mu}_a(t)\right)^2$.

%     \vspace{0.3cm}

%     \alt<2>{\begin{colorblock}{Theorem \gray[Audibert et al. 09]}
%             For a bandit instance with bounded rewards, UCB-V satisfies
%             \[\cR_{\bm\nu}\left(\texttt{UCB-V},T\right) \leq C \blue\left(\sum_{a : \mu_a < \mu_\star}\frac{\sigma_a^2}{\Delta_a}\right)\black\log(T)\]
...
%             \begin{equation*}
%                 \bP\Big(\frac{1}{n}\sum_{i=1}^n \big(X_i - \mu_i\big)  \geq \sqrt{\frac{2\hat \sigma_n^2\log(2/\delta)}{n}} + \frac{7\log(2/\delta)}{3n}\Big) \leq \delta
%             \end{equation*}
...
%     \begin{colorblock}{ISM-Normal \color{gray} [Cowan et al. 17] \black}
%         \[A_{t+1} = \argmax_{a=1,\dots,K} \ \red \widehat{\mu}_a(t) + \widehat{\sigma}_a(t) \sqrt{t^{\frac{2}{N_a(t) -2}}-1}\black.\]
%     \end{colorblock}
...
%     \[\cR_{\nu}(\texttt{ISM},T) \leq (1+\varepsilon)\!\!\!\!\!\!\!\!\underbrace{\blue\sum_{a : \mu_a < \mu_\star} \frac{2}{\log\left(1+ \frac{\Delta_a^2}{\sigma_a^2}\right)}\black}_{\substack{\text{optimal constant}\\\text{(Burnetas and Katehakis lower bound)}}}\!\!\!\!\!\!\!\!\log(T) + O_\varepsilon(\log\log(T)).\]

%     \begin{itemize}
%         \item[$\Longrightarrow$] asymptotic optimality beyond Gaussian rewards?
%     \end{itemize}

% \end{frameO}


% \section{Asymptotically Optimal Algorithms}
...
%     \end{itemize}
%     \textbf{Examples:} \textit{Bernoulli, Gaussian with known variance, Poisson, Exponential}
%     \vspace{0.3cm}

%     \textbf{Lai and Robbins lower bound:}
%     \[\liminf_{T\Longrightarrow \infty} \frac{\cR_{\bm\nu}(\cA,T)}{\log(T)} \geq \sum_{a : \mu_a < \mu_\star} \frac{\Delta_a}{\red\kl\black(\mu_a,\mu_\star)}.\]
...
%         For some parameter $c\geq 0$, use
%         \vspace{-0.5cm}
%         \[\UCB_{a}(t)=\max\left\{q : \red \kl\black\left(\widehat{\mu}_a(t),q\right) \leq \frac{\log(t)+c\log\log(t)}{N_a(t)}\right\},
%         \]
%     \end{colorblock}
...
%     \begin{center}
%         \includegraphics[height=4.5cm]{Interval.pdf}
%     \end{center}
...
%             We recover
%             \[\blue\UCB_a(t) = \widehat{\mu}_a(t) + \sqrt{\frac{2\sigma^2\left(\log(t) + c \log\log(t)\right)}{N_a(t)}}\black\]
%             \begin{itemize}
%                 \item[$\Longrightarrow$] upper-confidence bound on $\mu_a$
%             \end{itemize}

%         }}

%     % \begin{eqnarray*}
%     %  \bP\left(\mu_a > \UCB_{a,s}(\gamma) \right) & = & \bP\left(s\times \kl(\widehat{\mu}_{a,s} , \mu_a) > \gamma)\right) \\
%     %  & = & \bP\left(\widehat{\mu}_{a,s} > x_\gamma\right) \leq e^{-sd(x_\gamma,\mu)} = e^{-\gamma}
%     % \end{eqnarray*}
...
%     \begin{colorblock}{Theorem \gray[Cappé et al, 13]} If $c \geq 3$, for every arm such that $\mu_a < \mu_\star$,
%         \[\bE_{\bm\mu}[N_a(T)] \leq \frac{1}{\red\kl\black(\mu_a,\mu_\star)}\log(T) + C_{\bm\mu}\sqrt{\log(T)}.\]
%         \begin{center}
...
%     \end{itemize}
%     \[\cR_{\bm\mu}(\mathrm{kl}\text{-UCB},T) \simeq \blue \left(\sum_{a : \mu_a < \mu_\star} \frac{\Delta_a}{\kl(\mu_a,\mu_\star)}\right)\black\log(T).\]
...
%     \[\text{Bernoulli arms } \bm\mu = [0.1 \ 0.05 \ 0.05 \ 0.05 \ 0.02 \ 0.02 \ 0.02 \ 0.01 \ 0.01 \ 0.01]\]
...
%     \hfill {\small (Credit: Cappé et al, 13)}
% \end{frameO}

% \begin{frameO}[Where do the improvements come from?]

%     \begin{colorblock}{Theorem \gray[Cappé et al, 13]} If $c \geq 3$, for every arm such that $\mu_a < \mu_\star$,
%         \[\bE_{\bm\mu}[N_a(T)] \leq \frac{1}{\kl(\mu_a,\mu_\star)}\log(T) + C_{\bm\mu}\sqrt{\log(T)}.\]
%         \hfill{} \textit{(explicit constant in the paper)}
%     \end{colorblock}

%     \begin{itemize}
%         \item[$\Longrightarrow$] follows from  \red two improvements \black in the previous analysis
%     \end{itemize}
%     \alt<3>{\[\bE[N_a(T)] \leq \underbrace{\sum_{t=0}^{T-1}\bP(\UCB_1(t) \leq \mu_1)}_{A : \blue \text{ a better concentration result}} + \underbrace{\frac{f(T)}{\kl(\mu_a,\mu_1)} + \mathcal{O}(\sqrt{f(T)})}_{B : \red\text{ a finer upper bound}} \]}
...
%     \alt<2->{\hfill$f(T) = \log(T) + c \log\log(T)$}{}
% \end{frameO}
...
% %     \begin{eqnarray*}
% %         \bP(\UCB_1(t) \leq \mu_1) & = & \bP\left( N_1(t)\times \kl^+\!\!\left(\widehat{\mu}_1(t),\mu_1\right) > \log(t) + c\log\log(t)\right) \\
% %         & \leq & \bP\left(\exists s \leq t : s \times \kl^+(\widehat{\mu}_{1,s},\mu_1) > \log(t) + c\log\log(t)\right)
% %     \end{eqnarray*}

% %     \alt<3>{\textbf{Second idea:} \red peeling trick \black
% %     \begin{alertblock}{Lemma \gray[Garivier and Cappé, 2011]}
% %         \[\bP\left(\exists s \leq t : s \times \kl^+\left(\widehat{\mu}_{1,s},\mu_1\right) > \gamma \right) \leq e \lceil\gamma \log(t)\rceil  e^{-\gamma}.\]
% %     \end{alertblock}

% %     \vspace{-0.3cm}

% %     \begin{eqnarray*}
% %         \bP(\UCB_1(t) \leq \mu_1) & = & \mathcal{O}\left( \frac{\log^2(t)}{t\log^c(t)}\right)
% %     \end{eqnarray*}
...
% %     \begin{align*}
% %         \hspace{-0.4cm}\bP(\UCB_1(t) \leq \mu_1) & \!\leq\!\!\sum_{k=1}^{\frac{\log(t)}{\log(1+\eta)}} \bP\left( \exists s \in \cI_k , s\times \kl^+\!\!\left(\widehat{\mu}_{1,s},\mu_1\right)\! > \!\log(t) + c\log\log(t)\right)                                                                                             \\
% %                                                  & \leq \!\!  \sum_{k=1}^{\frac{\log(t)}{\log(1+\eta)}} \underbrace{\bP\left( \exists s \in \cI_k , s\times \kl^+\!\!\left(\widehat{\mu}_{1,s},\mu_1\right)\! > \!\log(t_{k}) + c\log\log(t_{k})\right)}_{\substack{\text{deviation of } \widehat{\mu}_{1,s} \text{ from its mean} \\\ \blue \text{uniformly over } s\in\cI_k}} \\
% %     \end{align*}
...
% %     \begin{eqnarray*}
% %         \bP(\UCB_1(t) \leq \mu_1) & = & \sum_{s=1}^t \bP\left( s\times \kl^+\!\!\left(\widehat{\mu}_{1,s},\mu_1\right) > \log(t) + c\log\log(t)\right) \\
% %         & \leq & \sum_{s=1}^t \frac{1}{t\log^c(t)} = \frac{1}{\log(t)^c}
% %     \end{eqnarray*}

% %     \[\rightsquigarrow \red  \sum_{t=1}^\infty \bP(\UCB_1(t) < \mu_1) = \infty\]
% %     \begin{itemize}
% %         \item[$\Longrightarrow$] not good enough\dots
% %     \end{itemize}
...
%               \begin{itemize}
%                   \item[$\Longrightarrow$] the Gaussian divergence $\kl(x,y) = 2(x-y)^2$ (UCB)
%                   \item[$\Longrightarrow$] the Bernoulli divergence $\kl(x,y) = \KL(\cB(x),\cB(y))$
%               \end{itemize}
...
%               \begin{itemize}
%                   \item[$\Longrightarrow$] distribution with a finite support \hfill \gray[Maillard et al. 11][Cappé et al. 13]\black

%                   \item[$\Longrightarrow$] exponential family with $d > 1$ parameters \hfill  \gray [Maillard, 17]
%               \end{itemize}
%     \end{itemize}
...
%               \begin{itemize}
%                   \item[$\Longrightarrow$] DMED, IMED \hfill \gray [Honda and Takemura, 10][Honda and Takemura, 16] \black
%                   \item[$\Longrightarrow$] empirical KL-UCB for bounded rewards \hfill \gray[Cappé et al. 13]\black
%               \end{itemize}
%     \end{itemize}
...
% % {\vspace{-0.5cm}
% %
% %  \[\color{gray} \text{[Cappé et al. 13]}\black: \ \ \ \bE_{\bm \mu}[N_k(T)]\leq  \color{blue}\frac{1}{d(\mu_k,\mu_\star)}\color{black}{\log T} + \mathcal{O}(\sqrt{\log(T)}).\]}
...
% %     \[A_{t+1} = \argmax_{a = 1,\dots,K} \ \red \widehat{\mu}_a(t) + \sqrt{\frac{\log_+\left(\frac{T}{KN_a(t)}\right)}{N_a(t)}}\]

% %     \begin{colorblock}{Theorem \gray [Audibert and Bubeck, 09]}
...
% %                   \[\cR_{\bm\nu}(\texttt{MOSS},T) \leq \alt<2>{\blue\frac{23K}{\Delta_{\min}}\black}{\frac{23K}{\Delta_{\min}}}\log\left(\max\left[\frac{110T\red\Delta_{\min}^2\black}{K},10^4\right]\right).\]\alt<2>{\vspace{-0.2cm}}{}
% %             \item It also holds that $\cR_{\bm\nu}(\texttt{MOSS},T) \leq 25\red \sqrt{KT}\black$.
...
% %     \alt<2>{\begin{itemize}
% %             \item[$\Longrightarrow$] \blue far from optimal \black in a problem-dependent sense
% %         \end{itemize}}{
% %         \begin{itemize}
% %             \item[$\Longrightarrow$] matching the worse-case lower bound in $\Omega(\sqrt{KT})$ !
% %         \end{itemize}}
...
% %     \textbf{Idea:} ``switch'' between KL-UCB and MOSS in order to be simultaneously \blue optimal \black in a problem-dependent and worst-case sense.
...
% %     \alt<2>{\begin{colorblock}{Theorem \gray [Garivier et al. 18]}
% %             Fix $\nu$ a bandit instance with bounded rewards.
% %             \begin{enumerate}
% %                 \item For all sub-optimal arm $a$,
% %                       \[\bE_{\nu}[N_a(T)] \leq \blue\frac{\log(T)}{\kl(\mu_a,\mu_\star)}\black + \mathcal{O}\left(\log^{2/3}(T)\right)\]
% %                 \item Moreover, $\cR_{\bm\nu}(\text{KL-UCB-Switch},T) \leq 25\blue\sqrt{KT}\black + (K-1)$.
% %             \end{enumerate}
% %         \end{colorblock}}{where \begin{colorblock}{}\vspace{-0.3cm}
% %             \begin{eqnarray*}
% %                 \UCB_a^{\text{KL}}(t)& = & \max \left\{ q : N_a(t) \times \kl\left(\widehat{\mu}_a(t), q\right) \leq \log_+\left(\blue\frac{T}{KN_a(t)}\black\right) \right\}\footnote{$\kl(x,y)=\kl_{\text{Ber}}(x,y)$; \text{can also rely on the non-parameteric KL-UCB index}},\\
% %                 \UCB_a^{\text{M}}(t)& = &\widehat{\mu}_a(t) + \sqrt{\frac{\log_+\left(\blue\frac{T}{KN_a(t)}\black\right)}{2N_a(t)}}.
% %             \end{eqnarray*}\end{colorblock}}
...
%     \begin{itemize}
%         \item Good \alert{concentration inequalities} are crucial to build good UCB algorithms!
%     \end{itemize}
...
% \section{A Bayesian Look at the Multi-Armed Bandit Model}

% \begin{frameTI}
%     \begin{center}
%         {\textcolor{white} {\Huge \textsc{A Bayesian Look at the MAB Model} }}
%     \end{center}
%     \vspace*{-4pt}
% \end{frameTI}
...
%             \Large \textbf{Two points of view}	\\[0.5cm]
...
%     \begin{minipage}{0.04\textwidth}\hspace{0.5cm}\end{minipage}\begin{minipage}{0.95\textwidth}{\footnotesize\begin{itemize}
%                 \item[1933] \orange Thompson: a Bayesian mechanism for clinical trials\black
%                 \item[1952] Robbins, formulation of the MAB problem
%                 \item[1956] \orange Bradt et al, Bellman: optimal solution of a Bayesian MAB problem \black
%                 \item[1979] \orange Gittins: first Bayesian index policy \black
%                 \item[1985] Lai and Robbins: lower bound, first asymptocally optimal algorithm
%                 \item[1985] \orange Berry and Fristedt: Bandit Problems, a survey on the Bayesian MAB \black
%                 \item[1987] Lai, asymptotic regret of \klUCB{} \orange + study of its Bayesian regret\black
%                 \item[1995] Agrawal, UCB algorithms
%                 \item[1995] Katehakis and Robbins, a UCB algorithm for Gaussian bandits
%                 \item[2002] Auer et al: UCB1 with finite-time regret bound
%                 \item[2009] UCB-V, MOSS\dots
%                 \item[2010] \orange Thompson Sampling is re-discovered \black
%                 \item[2011,13] Cappé et al: finite-time regret bound for \klUCB{}
%                 \item[2012,13] \orange Thompson Sampling is asymptotically optimal\black
%             \end{itemize}}
%     \end{minipage}
...
%     \begin{itemize}
%         \item Two probabilistic models \hfill{} two points of view!
%     \end{itemize}
...
%     % Y is the sequence of successive rewards obtained from arm a\dots
...
%             \\
%             (\red regret\black)
%              & (\red Bayes risk\black)
%             \\
%             \hline
%             \\
%             $\cR_{\bm\mu}(\cA,T)\! = \mathbb{E}_{\bm\mu}\!\!\left[\!\sum\limits_{t=1}^{T}\left(\mu_\star - \mu_{A_t}\right)\!\right]$
%              & $\texttt{R}^\pi(\cA,T)\! =\mathbb{E}_{\bm{\mu} \alert{\sim \pi}}\!\!\left[\!\sum\limits_{t=1}^{T}\left(\mu_\star - \mu_{A_t}\right)\!\right]$
%             \\
%              & \hspace{25pt} $ = \int \cR_{\bm\mu}(\cA,T) d\pi(\bm{\mu})$
%         \end{tabular}
%     \end{center}
...
%         \item[$\Longrightarrow$] \underline{posterior distribution}:
%               \vspace{-0.6cm}
...
%     $S_a(t) = \sum\limits_{s=1}^t R_s \ind_{(A_s = a)}$ sum of the rewards from arm $a$
...
% %     \hfill  $\Longrightarrow $ \blue Markov Decision Process with state $\Pi^t$
...
% %     \begin{colorblock}{}
% %         \[G_\gamma(p) = \inf \{ \lambda \in \R : V_\gamma_{\star}(p,\lambda)  = 0\},\]
% %         with
...
% %     \textbf{Finite-Horizon Gittins algorithm:}
...
% %         \item \dots with logarithmic regret \color{gray} [Lattimore, 2016]
% %     \end{itemize}
...
% %     \end{itemize}
% %     \[G(\pi_a(t-1),n) \simeq \max \left\{ q :  N_a(t)\times \kl\left(\widehat{\mu}_a(t),q\right) \leq \log \left(\color{red}\frac{n}{N_a(t)}\color{black}\right)\right\}\]
...
% %     \end{itemize}
% %     \[I_a(t)=\max \left\{ q : N_a(t) \times \kl\left(\widehat{\mu}_a(t),q\right) \leq \log\left(\color{red}\frac{T}{N_a(t)}\color{black}\right)\right\}\]
% %     \begin{itemize}
...
% %     \begin{itemize}
% %         \item[$\Longrightarrow$] looks like the \blue \klUCB{} index\black, with a \blue different exploration rate\black\dots
% %     \end{itemize}
...
%             \textcolor{darkgray}{\Large \textbf{Two points of view}}	\\[0.5cm]
%             % \textcolor{darkgray}{\Large \textbf{Insights from the Optimal Solution}}	\\[0.5cm]
...
%     \begin{itemize}
%         \item $\Pi_0=(\pi_1(0),\dots,\pi_K(0))$ be a prior distribution over $(\mu_1,\dots,\mu_K)$
%         \item $\Pi_t=(\pi_1(t),\dots, \pi_K(t))$ be the posterior distribution over the means $(\mu_1,\dots,\mu_K)$ after $t$ observations
%     \end{itemize}

%     \begin{colorblock}{}
%         The \textbf{Bayes-UCB algorithm} chooses at time $t$
%         \[
%             \color{red}A_{t+1} = \argmax_{a = 1,\dots,K} \  Q\left(1-\frac{1}{t(\log t )^c}, \pi_a(t)\right) \color{black}
%         \]
...
% \begin{frameO}[Bayes UCB in action \hfill{} (movie)]
...
%     \begin{colorblock}{Theorem \color{gray}[K.,Capp\'e,Garivier 2012]}

%         Let $\varepsilon > 0$. The Bayes-UCB algorithm using a uniform prior over the arms and parameter $c  \geq 5$ satisfies
%         \vspace{-0.3cm}
%         \[\mathbb{E}_{\bm\mu}[N_a(T)]\leq \frac{1+\varepsilon}{\kl(\mu_a,\mu_\star)}\log(T) + o_{\varepsilon,c}\left(\log(T)\right).\]
%         %\begin{align*}
%         % &  \bE[N_a(T)] \leq \frac{1+\varepsilon}{d(\mu_a,\mu_1)}\log(T) +
%         % \sqrt{\log T + 5\log\log T} \sqrt{\frac{2\pi(1+\varepsilon)^3d'(\mu_a,\mu_1)^2}{d(\mu_a,\mu_1)^3}} \\
%         %& \ \ \ \ \ \ \ \ \ + \left(\frac{1+\varepsilon}{d(\mu_a,\mu_1)} +
%         %\frac{2e+3}{1-\mu_1}\right)\log \log T + 27 +  2(1+\varepsilon)^2 \left(\frac{d'(\mu_a,\mu_1)}{d(\mu_a,\mu_1)}\right)^2.
%         %\end{align*}
%     \end{colorblock}
...
% %         where
% %         \begin{eqnarray*}
% %             u_a(t)\! & = &\!\! \max \left\{ q  : \kl \left(\frac{S_a(t)}{N_a(t)},q\right) \leq \frac{\log(t) +c\log(\log(t))}{N_a(t)}\right\} \\
% %             \tilde{u}_a(t) \!& = &\!\! \max\left\{ q : \kl\left(\frac{S_a(t)}{N_a(t)+1},q\right) \leq \frac{\log\left(\color{red}\frac{t}{N_a(t)+2}\color{black}\right) + c\log(\log(t))}{(N_a(t)+1)}\right\}
% %         \end{eqnarray*}
% %     \end{alertblock}
...
% %               \[
% %                   A_{t+1} = \underset{a}{\text{argmax}} \  Q\left(1-\frac{1}{t(\log t )^c}, \pi_{a,N_a(t),\widehat{\mu}_a(t)}\right)
% %               \]
% %     \end{itemize}
...
% %     \begin{colorblock}{Theorem \gray [K. 17]}
% %         Let $\overline{\mu}_a(t) = (\widehat{\mu}_a(t) \vee \mu^-)\wedge \mu^+$. The index policy

% %         \vspace{-0.3cm}

% %         \[
% %             A_{t+1} = \underset{a}{\text{argmax}} \   Q\left(1-\frac{1}{t(\log t )^c}, \pi_{a,N_a(t),\blue\overline{\mu}_a(t)\black}\right)
% %         \]
...
% %         \[\red\bE_{\bm \mu}[N_a(T)] \leq \frac{1+\varepsilon}{\kl(\mu_a,\mu_\star)}\log(T) + \mathcal{O}_{\varepsilon}(\sqrt{\log(T)})\black.\]
...
% %     \begin{colorblock}{\klUCB-H$^+$}
% %         \[u_a^{H,+}(t)=\max\left\{q :  N_a(t)\times \kl\left(\widehat{\mu}_a(t),q\right) \leq \log\left(\blue\frac{T\log^c T}{N_a(t)}\black\right)\right\}\]
% %     \end{colorblock}

% %     \begin{colorblock}{\klUCB$^+$}
% %         \[u_a^{+}(t)=\max\left\{q  :  N_a(t) \times \kl\left(\widehat{\mu}_a(t),q\right) \leq \log\left(\blue\frac{t\log^c t}{N_a(t)}\black\right)\right\}\]
% %     \end{colorblock}
...
%     \begin{colorblock}{Thompson Sampling: a randomized Bayesian algorithm}
%         \[
%             \left\{
%             \begin{array}{l}
...
%     \begin{colorblock}{Problem-dependent regret}
%         \[\forall \varepsilon>0, \ \ \bE_{\bm\mu}[N_a(T)] \leq \frac{\color{red}1+\varepsilon\color{black}}{\color{red}\kl(\mu_a,\mu_\star)\color{black}}\log(T) + o_{\mu,\varepsilon}(\log(T)).\]
...
%     \begin{colorblock}{Problem-independent regret \gray[Agrawal and Goyal 13]}
%         For Bernoulli and Gaussian bandits, Thompson Sampling satisfies

%         \vspace{-0.3cm}

%         \[\cR_{\bm\mu}(\texttt{TS},T) =O \left(\sqrt{KT\log(T)}\right).\]
...
%         $K=10$ Bernoulli arms bandit problem $\bm\mu=[0.1 \ 0.05 \ 0.05 \ 0.05 \ 0.02 \ 0.02 \ 0.02 \ 0.01 \ 0.01 \ 0.01]$
%     \end{center}
...
%     \begin{colorblock}{Two families of asymptotically optimal algorithms}
%         \begin{itemize}
...
%     \begin{colorblock}{BESA}
%         \begin{itemize}
%             \item Competitive regret against state-of-the-art for various $\cD$.
%             \item Same algorithm for all $\cD$.
%             \item Not relying on upper confidence bounds, not Bayesian\dots
%             \item \dots and extremely simple to implement.
%         \end{itemize}
%     \end{colorblock}


%     \begin{itemize}
%         \item[$\Longrightarrow$] How? Optimality? For which distributions ?
%     \end{itemize}
...
%     \begin{colorblock}{Theorem \gray[Baransi et al. 14]}
%         If $\exists \alpha \in(0,1), c>0$ such that $\alpha(M,1) \leq c\alpha^M$, then
%         \[
%             \cR_{\nu}(\texttt{BESA},T) \leq \frac{11\log(T)}{\mu_\star-\mu_a} + C_{\nu} + \mathcal{O}(1)\,.
%         \]
%     \end{colorblock}
%     %
...
%     \textbf{Future work:} understand when BESA fails, and whether it can be asymptotically optimal in some cases\dots
% \end{frameO}
...
%     \begin{center}
%         {\Large Can we achieve $\mathcal{O}(\sqrt{KT})$ regret with respect to the best static action if the rewards are arbitrarily generated?}
%     \end{center}
...
\section{Selfish MAB learning in IoT Netwokrs}

\begin{frameTI}
    \begin{center}
        {\textcolor{white} {\Huge \textsc{Selfish MAB learning in IoT Netwokrs} }}
    \end{center}
    % \vspace*{-4pt}

    \vfill{}
    \small{\textcolor{lightgray}{Ref: Bonnefoi et al, CROWNCOM 2017, best paper award.}}
\end{frameTI}
...
\begin{frameO}[Slides from IEEE WCNC 2019 demo ICT 2018]
    TODO

    \begin{center}
        \movie{\includegraphics[width=0.9\textwidth]{screenshotDemoYouTube.png}}{videos/Multi-Armed_bandit_Learning_in_Iot_Networks_MALIN_-_Demo_at_ICT_2018.mkv}
    \end{center}
\end{frameO}


\section{Other Bandit Models}

\begin{frameTI}
    \begin{center}
        {\textcolor{white} {\Huge \textsc{Other Bandit Models} }}
    \end{center}
    \vspace*{-4pt}
\end{frameTI}
...
% \begin{frameO}[\emph{Structured/combinatorial} stochastic bandits]

%   \begin{itemize}
%     \item Arms are edges on a graph
%     \item $\mathcal{M}$ is a set of possible configurations (subsets of edges)
%     \item The agent chooses $m_t \in \mathcal{M}$ at time $t$ \\
%       and then observes a realization of all arms in $\mathcal{M}$
%     \item Goal: discover the best configuration $m^*\in\mathcal{M}$
%   \end{itemize}


%   \begin{center}
%     \includegraphics[height=3cm]{assignements}
%   \end{center}

%   \textbf{Example}: find a matching between users and channels/antennas
%   (centralized decision-making like in Wi-Fi).

%   \hfill \color{gray} [Lelarge et al., 13]
...
% \subsection{Relaxing the i.i.d. assumption}

% \begin{frame}{A restless bandit example}

%   % orammy: \[(X_{k,t}) \ \ \ \text{is some stochastic process}\]

%   Restless Markov bandit
%   \[\text{for all } k, \ \ (X_{k,t})_{t \in \N} \ \ \text{is a \red Markov chain} \]


%   \underline{\orange Cognitive radio}:
%   \begin{itemize}
%     \item the behavior of primary users is evolving according to a Markovian dynamic
%     \item simple model: state space $\{0,1\}$ occupied/available
%   \end{itemize}
...
%   \textbf{Idea:} If arm $k$ has \textit{stationnary distribution } $\pi_k$, aim to always select the channel
%   \[\red \underset{k \in \{1,\dots,K\}}{\text{argmax}} \ \bE_{X\sim\pi_k} [X]\]

%   \begin{itemize}
%     \item[$\Longrightarrow$] may be a bad idea\dots
%   \end{itemize}
...
%   \textbf{Example}: a transition matrix on $\{0,1\}$
...
%   2-armed bandit:
%   \begin{itemize}
%     \item arm 1 is a Markov chain with transition $P_{\epsilon_1}$
%     \item arm 2 is a Markov chain with transition $P_{\epsilon_2}$
%   \end{itemize}
...
%   \textbf{Strategies}:
%   \begin{itemize}
%     \item static strategy playing a single arm $\Longrightarrow$ average reward $1/2$
%     \item a much better strategy when $\epsilon_1$ and $\epsilon_2$ are small:
...
%   \begin{itemize}
%     \item[$\Longrightarrow$] regret with respect to the best static action is no longer (always) the right notion
%   \end{itemize}

%   \vspace{-0.3cm}

%   \color{gray} \hfill [Ryabko et al. 2014] \color{black}

% \end{frame}

% \begin{frame}{Some tools for restless bandits}


%   \begin{itemize}
%     \item Bayesian approaches based on Whittle indices
%           \color{gray}

%           Liu and Zhao,
%           Indexability of Restless Bandit Problems and Optimality of Whittle Index for Dynamic Multichannel Access. I.T., 2010\color{black}

%     \item Using reinforcement learning algorithms

%           (restless Markov bandit = a Markov Decision Process)

%           \color{gray}

%           Ortner, Ryabko, Auer and Munos, Regret bounds for restless Markov bandits, TCS, 2014\color{black}

%     \item Can we modify the UCB approach?

%           \color{gray}

%           Liu et al, Learning in a Changing World: Restless Multiarmed Bandit With Unknown Dynamics. IEEE I.T., 2013

%   \end{itemize}


%   \underline{\orange Experiments}:

%   \begin{itemize}
%     \item plain UCB may still be robust on some Markovian arms
%   \end{itemize}
...
% \subsection{Relaxing the stochastic assumption}


% \begin{frame}{A new bandit game}

%   At round $t$

%   \begin{itemize}
%     \item the player chooses arm $A_t$
%     \item simultaneously, an \red adversary\black \ chooses the vector of rewards \[(x_{1,t},\dots,x_{K,t})\]
%     \item the player receives the reward $x_t = x_{A_t,t}$
%   \end{itemize}

%   \vspace{0.3cm}

%   \textbf{Goal}: maximize rewards, or minimize \orange regret\black

%   \[\mathrm{R}(T) = \max_{a} \bE\left[\sum_{t=1}^T x_{a,t}\right] - \bE\left[\sum_{t=1}^T x_{t}\right].\]

% \end{frame}



% \begin{frame}{Exponential Weighted Forecaster}

%   \textbf{The full-information game}: at round $t$

%   \begin{itemize}
%     \item the player chooses arm $A_t$
%     \item simultaneously, an \red adversary\black \ chooses the vector of rewards \[(x_{t,1},\dots,x_{t,K})\]
%     \item the player receives the reward $x_t = x_{A_t,t}$
%     \item \orange and he observes the reward vector $(x_{t,1},\dots,x_{t,K})$
%   \end{itemize}

%   \textbf{The EWF algorithm \color{gray}[Littelstone, Warmuth 1994]}

%   With $\widehat{p}_t$ the probability distribution
%   \[\widehat{p}_{t}(k) \propto e^{\eta \left(\sum_{s=1}^{t-1} x_{k,s}\right)}\]

%   \vspace{-0.3cm}

%   at round $t$, choose
%   \[\red A_t \sim \widehat{p}_{t}\black \]
...
% \begin{frame}{The EXP3 strategy}

%   We don't have access to the $(x_{k,t})$ for all $k$\dots

%   \[\red \widehat{x}_{k,t} = \frac{x_{k,t}}{\widehat{p}_{k,t}}\ind_{(A_t=k)}\]

%   \vspace{-0.3cm}

%   satisfies $\bE[\widehat{x}_{k,t}] = x_{a,t}$.

%   \vspace{0.3cm}

%   \textbf{The EXP3 algorithm} %\color{gray}[Auer et al. 2003]\color{black}

%   \vspace{0.2cm}

%   With $\widehat{p}_t$ the probability distribution
%   \[\widehat{p}_{t}(k) \propto e^{\eta \left(\sum_{s=1}^{t-1} \orange \widehat{x}_{k,s}\black\right)}\]

%   \vspace{-0.3cm}

%   at round $t$, choose
%   \[\red A_t \sim \widehat{p}_{t} \]

%   \color{gray}

%   Auer, Cesa-Bianchi, Freund, Schapire, \emph{The nonstochastic multiarmed bandit problem}, SIAM J. Comput., 2002
...
%   \textbf{The EXP3 strategy} %\color{gray}[Auer et al. 2003]\color{black}
...
%   \textbf{Theorem} \color{gray} [Bubeck and Cesa-Bianchi 12]\black

%   EXP3 with \[\eta = \sqrt{\frac{\log(K)}{KT}}\]
%   satisfies
%   \[R(T) \leq  \sqrt{2 \log K}\red \sqrt{KT}\]
...
\begin{frameO}[Multi-players bandits: setup]
    ...
    \orange $M$ players \black playing \textit{the same} $K$-armed bandit \ \ ($2 \leq M \leq K$)
...
\begin{frameO}[Multi-players bandits: algorithms]
    ...
    \only<1>{
        \begin{colorblock}{\textbf{Example:} UCB1 + $\rho^{\text{rand}}$. At round $t$ each player}
            \begin{itemize}
                ...
                \item \alert{their proof was wrong\dots}
    ...
    \only<2->{
        \begin{colorblock}{\textbf{Example:} our algorithm \textcolor{blue}{klUCB index + MC-TopM rule}}
            \begin{itemize}
                \only<2>{
                \item more complicated behavior (musical chair game)
                \item we obtain a $\cR(\cA, T) = \mathcal{O}(M^3 \frac{1}{\Delta_M^2} \log(T))$ regret upper bound
                \item lower bound is $\cR(\cA, T) = \Omega(M \frac{1}{\Delta_M^2} \log(T))$
                \item order optimal, not asymptotically optimal
                }
                \item \textbf{Recent references}: \color{gray} [Besson and Kaufmann, 18] [Boursier et al, 19]
            \end{itemize}
        \end{colorblock}
    }
    ...
    \only<3>{
        \textbf{Remarks}:
        \begin{itemize}
            \item number of players $M$ has to be known\\
            ...
            \item is it a \textit{fair} orthogonalization rule?
...
\begin{frameO}[Results on a multi-player MAB problem]
    ...
    \caption{\footnotesize{For $M=6$ objects, our strategy (MC-TopM) largely outperform SIC-MMAB and $\rho^{\text{rand}}$. \textcolor{cyan}{MCTopM + klUCB} achieves the best performance (among decentralized algorithms) !}}
...
\begin{frameO}[Piece-wise stationary bandits]
    ...
    \begin{colorblock}{\textbf{Piece-wise stationary} problems!}
        $\hookrightarrow$ the litterature usually focuses on the easier case,
        when there are at most $Y_T = o(\sqrt{T})$ intervals, on which the means are all stationary.
    \end{colorblock}
\end{frameO}
...
\begin{frameO}[Example of a piece-wise stationary MAB problem]
    ...
    of $K=3$ arms.
    There are $Y_T=4$ break-points and $5$ sequences
...
\begin{frameO}[Regret for piece-wise stationary bandits]
    ...
    (which changes between the $Y_T \geq 1$ stationary sequences)
    %
    ...
    \begin{colorblock}{Typical regimes for piece-wise stationary bandits}
        \begin{itemize}
            \item
                  The lower-bound is
                  $\cR(\cA,T) \geq \Omega(\sqrt{K T Y_T})$

            \item
                  Currently, state-of-the-art algorithms $\mathcal{A}$ obtain
                  \begin{itemize}\tightlist
                      \item
                            $\cR(\cA,T) \leq \mathcal{O}(K \sqrt{T Y_T \log(T)})$
                            if $T$ and $Y_T$ are known
                      \item
                            $\cR(\cA,T) \leq \mathcal{O}(K \textcolor{orange}{Y_T} \sqrt{T \log(T)})$
                            if $T$ and \textcolor{orange}{$Y_T$ are unknown}
                  \end{itemize}
            \item
...
\begin{frameO}[Results on a piece-wise stationary MAB problem]
    ...
    \textbf{Idea:} combine a good \textit{bandit algorithm} with an \emph{break-point detector}
...
\begin{frameO}[Summary \hfill{} ($1/2$)]
...
\begin{frameO}[$\hookrightarrow$ \href{https://smpybandits.github.io/}{\textcolor{blue}{\texttt{SMPyBandits.GitHub.io}}}]
