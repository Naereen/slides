...
\begin{frameO}[Who am I ?]
    ...
    \begin{itemize}
        ...
        \item under supervision of Prof. Christophe Moy at SCEE team, IETR \& CentraleSupélec in Rennes (France)
    ...
    \begin{itemize}
        \item Email : \href{https://perso.crans.org/besson/}{\textcolor{darkgreen}{\texttt{Lilian.Besson @ CentraleSupelec.fr}}}
...
\begin{frameO}[Spectrum scarcity: there are not enough free bands]
    \begin{itemize}
        ...
        \item
        $\implies$ Almost all frequencies are registered and their usage is limited!
    \end{itemize}
    \begin{center}
        \includegraphics[width=0.70\textwidth]{United_States_Frequency_Allocations_Chart_2016_The_Radio_Spectrum_3}
    \end{center}
\end{frameO}


\begin{frameO}[Non homogeneous frequency usage]
    But\dots almost everywhere and at anytime in the world, some radio channels in any standards are not used!
    \begin{center}
...
\begin{frameO}[Chapters of my thesis addressed today]
    ...
    \begin{enumerate}[leftmargin=50pt]
        ...
        \item[\textcolor{lightgray}{Ch.3)}] \textcolor{lightgray}{SMPyBandits: an exhaustive Python library to simulate MAB problems}
        \item[\textcolor{lightgray}{Ch.4)}] \textcolor{lightgray}{Expert aggregation for online MAB algorithms selection}
...
\begin{frameO}[Outline of this talk]
    ...
    \begin{itemize}
        ...
        \item \textcolor{lightgray}{Spectrum issues in wireless networks}
...
\begin{frameO}[Other asymptotic lower bounds]
    ...
    Such asymptotic lower bound can be generalized to extensions of the classical stationary single-player MAB model

    \vspace*{10pt}

    \begin{colorblock}{For multi-player bandit}
        Same lower-bound, with a problem-dependent constant $C$
        \[ R_T = \Omega(C M K \log(T)) \]
    \end{colorblock}

    \vspace*{10pt}

    \begin{colorblock}{For piece-wise stationary bandit}
        Larger lower-bound, with a problem-dependent constant $C$
        \[ R_T = \Omega(C \sqrt{K \Upsilon_T T}) \]
    \end{colorblock}

    \textcolor{gray}{(see later)}
...
\begin{frameO}[Regret of UCB($\alpha$) for bounded rewards]
    ...
    In most of our work, we use an extension of UCB called kl-UCB.
    ...
    \begin{colorblock}{Theorem \hfill{} \gray[Garivier et al, 2013]\black}
        For bounded rewards, kl-UCB satisfies

        \[\cR_{\bm\nu}(\texttt{kl-UCB},T) \leq \left(\sum_{a : \mu_a < \mu_\star} \frac{\Delta_a}{\texttt{kl}(\mu_a, \mu_\star)}\right)\log(T) + o(\log(T)).\]
    \end{colorblock}
...
\begin{frameO}[We implemented this with real hardware ($1/2$)]
    \begin{center}
        \movie{\includegraphics[width=0.9\textwidth]{screenshotDemoYouTube.png}}{videos/Partie1.mp4}
    \end{center}
\end{frameO}


\begin{frameO}[We implemented this with real hardware ($2/2$)]
    \begin{center}
        \movie{\includegraphics[width=0.9\textwidth]{screenshotDemoYouTube.png}}{videos/Partie2.mp4}
    \end{center}
\end{frameO}


\section{Extensions to other Bandit Models}
...
\begin{frameO}[Results on a multi-player MAB problem ($1/3$)]
    ...
    \centering
    \includegraphics[height=0.80\textheight]{MP__K9_M9_T10000_N200__4_algos/all_RegretCentralized____env1-1_2306423191427933958.pdf}
...
\begin{frameO}[Results on a multi-player MAB problem ($2/3$)]
    ...
    \centering
    \includegraphics[height=0.80\textheight]{MP__K9_M6_T5000_N500__4_algos/all_RegretCentralized____env1-1_8318947830261751207.pdf}
    \caption{\footnotesize{For $M=6$ objects, our strategy (MC-TopM) largely outperform previous state-of-the-art and $\rho^{\text{rand}}$.}}
...
\begin{frameO}[Conclusion: summary \hfill{} ($1/2$)]
    ...
    \begin{darkblock}{Didn't have time to talk about\dots}
        \begin{itemize}
            \setlength\itemsep{5pt}
            \item my Python library SMPyBandits (Chapter 3),
            \item aggregation of bandit algorithms (Chapter 4),
            \item and details about our theoretical results and proofs (Chapters 6 and 7).
        \end{itemize}
    \end{darkblock}
...
\begin{frameO}[Conclusion: perspectives \hfill{} ($2/2$)]
    ...
    \begin{lightblock}{What could be done with another year(s) ?}
        ...
        \begin{itemize}
            ...
            \item unify the multi-player and non-stationary bandit models\\
                $\hookrightarrow$ in progress, already a paper doing it, we can do better with our tools
            \item test the whole framework and our proposed algorithms in real-world environment\\
                $\hookrightarrow$ was started in summer $2019$ with an intern working with Christophe Moy
            \vspace*{10pt}
            \item the ``Graal'' goal:
                try to also let the devices learn \emph{when} to transmit, not only \emph{where/how} to transmit\dots\\
