...
\begin{frameO}[The MC-Top-M algorithm]
    ...
    \begin{figure}[h!]
        ...
        \begin{tikzpicture}[>=latex',line join=bevel,scale=5.5]
            ...
            % \pause
            \path [color=blue,very thick,->] (notfixed) edge[loop right] node[right,text width=4cm,text badly centered,black] {\small \textcolor{blue}{$\bold{(2)}$}  $C^j(t), A^j(t) \in \hat{M}^{m}(t)$} (1);
            % \pause
            ...
            \draw [color=cyan,very thick,->] (notfixed) to[bend right] node[midway,above,text width=5cm,text centered,black] {\small \textcolor{cyan}{$\bold{(1)}$} $\overline{C^j(t)}, A^j(t) \in \hat{M}^{m}(t)$} (fixed);
            % \pause
            \path [color=darkgreen,very thick,->] (fixed) edge[loop left] node[left,text width=2.9cm,text badly centered,black] {\small \textcolor{darkgreen}{$\bold{(4)}$} $A^j(t) \in \hat{M}^{m}(t)$} (fixed);
            % \pause
...
\begin{frameO}[Theoretical results]
    ...
    \begin{darkblock}{Control of the \textcolor{orange}{sub-optimal selections} \hfill{} \textcolor{gray}{[Besson \& Kaufmann, 18]}
    \only<1>{
    ...
    \begin{colorblock}{Control of the \textcolor{orange}{collisions} \hfill{} \textcolor{gray}{[Besson \& Kaufmann, 18]}
    \only<1>{
        \[
    ...
    \only<2>{
    \begin{lightblock}{$\implies$ \textbf{logarithmic regret!} \hfill{} \textcolor{gray}{[Besson \& Kaufmann, 18]}
        \begin{align*}
            R_{\bm\mu}(\cA,T)
...
\begin{frameO}[Piece-wise stationary bandits]
    ...
    \begin{blueblock}{Stationary MAB problems}
...
\begin{frameO}[Regret for piece-wise stationary bandits]
    ...
    %
    \[ \alert{ \cR(\cA,T) } = \mathbb{E}\left[ \sum\limits_{t=1}^T r_{\alert{k^*(t)}}(t) \right] - \sum\limits_{t=1}^T \mathbb{E}\left[ r(t) \right] = \underbrace{\left(\alert{\sum_{t=1}^T \max_k \mu_k(t)} \right)}_{\text{oracle total reward}} - \sum\limits_{t=1}^T \mathbb{E}\left[ r(t) \right]. \]
    ...
    \begin{colorblock}{Typical regime for piece-wise stationary bandits}
        \begin{itemize}
            ...
            % on a ``bad'' problem
            \item State-of-the-art algorithms $\mathcal{A}$ can obtain $\cR(\cA,T) \leq \mathcal{O}(K \sqrt{T \Upsilon_T})$
            % \item
...
\begin{frameO}[Our new algorithm: klUCB index + BGLR detector]
    ...
    \pause
    % \only<2>{
    \begin{lightblock}{Regret bound (if $T$ and $\Upsilon_T$ are known)}
...
\begin{frameO}[Perspectives \hfill{} ($2/3$)]
    ...
    % \begin{lightblock}{What could be done with another year(s) ?}
        ...
        \begin{itemize}[<+->]
            ...
            \begin{itemize}
                \item propose a more realistic model for IoT networks (exogenous activation, non stationary traffic, etc)
