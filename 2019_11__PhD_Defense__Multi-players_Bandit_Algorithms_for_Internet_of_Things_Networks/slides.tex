...
\begin{frameO}[What did I study during my PhD ? ($1/2$)]
    ...
    \begin{darkblock}{Telecommunications technology\dots}
        ...
        \hspace{15pt} $\hookrightarrow$ \textbf{\alert{some/many} wireless devices} access a wireless network\\
        ...
        \hspace{35pt} $\hookrightarrow$ we focus on the case of \textbf{\alert{Internet of Things}} networks (IoT)
    \end{darkblock}
...
\begin{frameO}[Organization of the thesis: our reading map for today]
    ...
    \begin{figure}[h!]
        ...
        \begin{tikzpicture}[>=latex',line join=bevel,scale=2.25]
            ...
            \node[align=center] (chapter2) at (0,2.25) [rectangle,draw,fill=gray!10,text=gray] {\textbf{Chapter~2}\\The Stochastic\\Multi-Armed Bandit models};
...
\begin{frameO}[Outline of this talk]
    ...
    \begin{itemize}
        ...
        \item \textcolor{gray}{\sout{Multi-armed Bandit}}
        % \begin{itemize}
...
\begin{frameTI}
    ...
    \vspace*{10pt}
    \begin{center}
        {\textcolor{white} {\Huge \textsc{Selfish MAB Learning in IoT Networks} }}
    \end{center}
    % \vspace*{-4pt}
    \vfill{}\hfill{}
    \small{\textcolor{lightgray}{Ref: Chapter 5.2 of my thesis, and [Bonnefoi, Besson et al, 2017].}}
...
\begin{frameO}[We implemented this with real hardware ($1/3$)]
    ...
    \begin{center}
        \movie{\includegraphics[width=0.7\textwidth]{screenshotDemoYouTube.png}}{videos/demoICT2018.mkv}
    \end{center}
    {\footnotesize \textcolor{gray}{[Bonnefoi et al, ICT 2018], [Besson et al, WCNC 2019], Ch.5.3}}
\end{frameO}
...
\begin{frameO}[Conclusion from part I : Selfish MAB learning for IoT]
    ...
    \begin{itemize}
        ...
        \item
            Only with a stationary hypothesis on the background traffic\dots
        \item
            in average at least $p \times D$ dynamic devices, using $K$ channels\\
            $\implies$ $p \leq \frac{K}{D}$ or $D \leq \frac{K}{p}$ gives best performance
        \item
            \textbf{Intractable model} in theory
    \end{itemize}

    \vfill{}
    \textcolor{orange}{We can study theoretically two more specific models, in two directions}
    \begin{itemize}
        \item multi-player bandits: devices are always activated\\
            $p=1$ $\implies$ $D = M \leq \frac{K}{p} = K$)
        \item non-stationary bandits
    \end{itemize}
    \hfill{}
    $\implies$ Both are tractable models!

\end{frameO}


\section{Extensions with other Bandit Models}
...
\begin{frameTI}
    ...
    \vspace*{10pt}
    \begin{center}
        {\textcolor{white} {\Huge \textsc{Extensions with other Bandit Models} }}
    \end{center}
    % \vspace*{-4pt}
    \vfill{}
    \small{\textcolor{lightgray}{Ref: Chapters 6 and 7 of my thesis\newline
    and [Besson \& Kaufmann, 2018]
    and [Besson \& Kaufmann, 2019].}}
    % \hfill{} {\tiny \textcolor{gray}{[Bonnefoi, Besson et al, CROWNCOM 2017], Ch.5.2}}
\end{frameTI}
...
\begin{frame}[c]
    \begin{changemargin}{-0.5cm}{-0.5cm}
        \begin{center}
            \vspace{-0.3in}
            \textbf{\huge Extensions with other Bandit Models}
            \vspace{1.5cm}
...
\begin{frame}[c]
    \begin{changemargin}{-0.5cm}{-0.5cm}
        \begin{center}
            ...
            % \hfill{}
            \tiny{\textcolor{gray}{Ref: Chapter 6 of my thesis, and [Besson \& Kaufmann, 2018].}}
            \\[0.5cm]
            \textcolor{darkgray}{\textbf{\Large Piece-wise stationary bandits}}
        \end{center}
    \end{changemargin}
...
\begin{frameO}[M-P bandits for Cognitive Radios: previous model]

    \begin{colorblock}{``Listen before talk''
        \hfill{} {\tiny \textcolor{lightgray}{[Zhao et al. 2010], [Jouini et al. 2010], [Anandkumar et al. 2011]}}
    }
        \begin{itemize}
            \item
            ...
            \begin{itemize}
                \item
                observe first $X_{A_{m,t}, t}$,
                \item
                if $X_{A_{m,t}, t} = 1$, transmit and then observe the joint $X_{m,t}$,
                \item
...
\begin{frameO}[M-P bandits for Cognitive Radios: our models]
    ...
    \begin{colorblock}{``Talk and maybe collide''
        ...
        \begin{itemize}
            ...
            \item
            Feedback model: observe only the joint information $X_{m, t}$.\\
            No collision if $\neq 0$, but cannot know if collision or zero reward if $= 0$?

            \item
            \textcolor{lightgray}{We studied an extension with retransmissions in [Bonnefoi et al, WCNC 2019], Ch.5.4.}
        \end{itemize}
    \end{colorblock}

    \begin{colorblock}{``Observe collision then talk?''
        \hfill{} \textcolor{lightgray}{[Boursier et al, 2019]}
    }
        \begin{itemize}
            \item
...
\begin{frameO}[Multi-players bandits: algorithms]
    ...
    \only<1>{
        \begin{colorblock}{Old idea: UCB1 + $\rho^{\text{rand}}$. At round $t$ each player $m\in\{1,\dots,M\}$}
            \begin{itemize}
...
\begin{frameO}[Results on a multi-player MAB problem ($1/2$)]
    ...
    \caption{\footnotesize{For $K=M$ objects, our strategy (MCTopM) achieves \textbf{constant} regret!}}
...
\begin{frameO}[Results on a multi-player MAB problem ($2/2$)]
    ...
    \caption{\small{For example for $M=6$ objects, our strategy (MCTopM) largely outperforms $\rho^{\text{rand}}$ and other previous state-of-the-art policies (not included).}}
...
\begin{frame}[shrink=20]
    ...
    \begin{center}
        \begin{tabular}{ m{2.7cm} | c | m{2.7cm} | m{2cm} | m{1.5cm} | m{3cm} }
            \textbf{Algorithm} & \textcolor{gray}{Paper} & \textbf{Regret bound}
            & \textbf{Efficiency} \newline {\footnotesize \textcolor{gray}{$\star\star\star\star$ is better}}
            & \textbf{Cost} \newline {\footnotesize \textcolor{gray}{€ is better}}
            & \textbf{Parameters} \\
            \hline
            Centralized multiple-play klUCB & \textcolor{gray}{[1]} & $\textcolor{blue}{\mathbf{C_{M,\mu}}} \log(T)$ & {\Large \textcolor{orange}{$\star\star\star\star$}} & {\textcolor{orange}{€}} & {\footnotesize $M$ \textcolor{red}{in another model} } \\
            % \vspace*{3pt}
            \hline
            % \vspace*{3pt}
            $\rho^{\text{rand}}$ UCB & \textcolor{gray}{[2]} & $M^3 \textcolor{red}{\mathbf{C}'} \log(T)$ & {\Large \textcolor{orange}{$\star\star$}} & {\textcolor{orange}{€}} & {\footnotesize just $M$ } \\
            \hline
            \textsc{MEGA} & \textcolor{gray}{[3]} & $\textcolor{red}{T^{3/4}}$ & {\Large \textcolor{orange}{$\star$}} & {\textcolor{orange}{€}} & {\footnotesize 4 par., \emph{impossible to tune} } \\
            \hline
            Musical Chair & \textcolor{gray}{[4]} & ${2M \choose M} \textcolor{red}{\mathbf{C}'} \log(T)$ & {\Large \textcolor{orange}{$\star\star$}} & {\textcolor{orange}{€}} & {\footnotesize 1 parameter $T_0$ \newline \emph{hard to tune} } \\
            \hline
            Selfish UCB & \textcolor{gray}{[5]} & $\textcolor{red}{T}$ in some case & {\Large \textcolor{orange}{$\star$}} & {\textcolor{orange}{€}} & \textbf{none} \\
            % \vspace*{3pt}
            \hline
            % \vspace*{3pt}
            \textcolor{blue}{\small \textbf{MCTopM klUCB}} & \textcolor{gray}{[6]} & $M^2 \textcolor{blue}{\mathbf{C_{M,\mu}}} \log(T)$ & {\Large \textcolor{orange}{$\star\star\star$}} & {\textcolor{orange}{€€}} & just $M$ \\
            \hline
            \textsc{Sic-MMAB} & \textcolor{gray}{[7]} & $\textcolor{blue}{\mathbf{C_{M,\mu}}} \log(T)$ & {\Large \textcolor{orange}{$\star\star\star$}} & {\textcolor{orange}{€€€}} & \textbf{none}
            % \\
            %  & & & \textcolor{gray}{($\star\star\star\star$ is better)} & \textcolor{gray}{(€ is better)} &
        \end{tabular}
    \end{center}
    ...
    Optimal \textbf{regret bound} is $\cR(\cA, T) \leq \textcolor{blue}{\mathbf{C_{M,\mu}}} \log(T) + o(\log(T))$,
    with $\textcolor{blue}{\mathbf{C_{M,\mu}}} = \sum\limits_{k: \mu_k < \mu_{M}^*} \sum\limits_{j=1}^M \frac{\mu_{M}^*}{\mathrm{kl}(\mu_k,\mu_j^*)}$,
    %
    and $\textcolor{red}{\mathbf{C}'} \gg \textcolor{blue}{\mathbf{C_{M,\mu}}}$ are much larger constants.
    ...
    \begin{small}
        \textbf{Ref:}
            ...
            [3] \textcolor{gray}{[Avner \& Mannor, 15]}
            [4] \textcolor{gray}{[Rosenski et al. 15]}
            [5] \textcolor{gray}{[Bonnefoi et al. 17]}
            [6] \textcolor{gray}{[Besson \& Kaufmann, 18]}
...
\begin{frame}[c]
    \begin{changemargin}{-0.5cm}{-0.5cm}
        \begin{center}
            ...
            % \hfill{}
            \tiny{\textcolor{gray}{Ref: Chapter 7 of my thesis, and [Besson \& Kaufmann, 2019].}}
        \end{center}
    \end{changemargin}
\end{frame}
...
\begin{frameO}[Regret for piece-wise stationary bandits]
    ...
    The ``oracle'' plays the (unknown) best arm $k^*(t) = \argmax \mu_k(t)$\\
    ...
    \begin{colorblock}{Typical regimes for piece-wise stationary bandits}
        \begin{itemize}
            ...
            \item
                  ...
                  \hfill{} \gray [Besson \& Kaufmann, 2019] \texttt{arXiv:1902.01575}
        \end{itemize}
    \end{colorblock}
...
\begin{frame}[shrink=25]
    ...
    \begin{tabular}{ m{3.5cm} | c | m{3.3cm} | m{2cm} | m{1.5cm} | m{3cm} }
        \textbf{Algorithm} & \textcolor{gray}{Paper} & \textbf{Regret bound}
        & \textbf{Efficiency} \newline {\footnotesize \textcolor{gray}{$\star\star\star\star$ is better}}
        & \textbf{Cost} \newline {\footnotesize \textcolor{gray}{€ is better}}
        & \textbf{Parameters} \\
        \hline
        Oracle-Restart UCB & \textcolor{gray}{[1]} & $\textcolor{blue}{\mathbf{C}} \Upsilon_T \log(T)$ & {\Large \textcolor{orange}{$\star$}} & {\textcolor{orange}{€}} & \textcolor{red}{unrealistic oracle} \\
        Naive UCB & \textcolor{gray}{[1]} & \textcolor{red}{$T$} \emph{in worst case} & {\Large \textcolor{orange}{$\star$}}  & {\textcolor{orange}{€}} & \textbf{none} \\
        \hline
        Discounted UCB & \textcolor{gray}{[2]} & $\textcolor{red}{\mathbf{C}'} \sqrt{T \Upsilon_T} \textcolor{red}{\log(T)}$ & {\Large \textcolor{orange}{$\star$}}  & {\textcolor{orange}{€}} & needs $\Upsilon_T$ \\
        Sliding-Window UCB & \textcolor{gray}{[2]} & $\textcolor{red}{\mathbf{C}'} \sqrt{T \Upsilon_T \log(T)}$ & {\Large \textcolor{orange}{$\star$}}  & {\textcolor{orange}{€}} & needs $\Upsilon_T$ \\
        \hline
        Discounted TS & \textcolor{gray}{[3]} & \emph{not yet proved} & {\Large \textcolor{orange}{$\star\star$}}  & {\textcolor{orange}{€}} & {\footnotesize how to tune $\gamma$ ?} \\
        Exp3.S & \textcolor{gray}{[4]} & $\textcolor{blue}{\mathbf{C}} \sqrt{T \Upsilon_T \log(T)}$ & {\Large \textcolor{orange}{$\star$}} & {\textcolor{orange}{€}} & needs $\Upsilon_T$ \\
        \hline
        CUSUM-UCB & \textcolor{gray}{[5]} & $\textcolor{red}{\mathbf{C}'} \sqrt{T \Upsilon_T \log(T / \Upsilon_T)}$ & {\Large \textcolor{orange}{$\star\star\star$}}  & {\textcolor{orange}{€€€}}& {\footnotesize needs $\Upsilon_T$ and $\delta_{\min}$} \\
        % \hline
        M-UCB & \textcolor{gray}{[6]} & $\textcolor{red}{\mathbf{C}'} \sqrt{T \Upsilon_T \log(T)}$ & {\Large \textcolor{orange}{$\star\star$}} & {\textcolor{orange}{€€}} & {\footnotesize needs $\Upsilon_T$ and $\delta_{\min}$} \\
        \hline
        \textbf{\textcolor{blue}{BGLR + kl-UCB}} & \textcolor{gray}{[7]} & $\textcolor{blue}{\mathbf{C}} \sqrt{T \Upsilon_T \log(T)}$ & {\Large \textcolor{orange}{$\star\star\star\star$}} & {\textcolor{orange}{€€€}} & needs $\Upsilon_T$ \\
        \hline
        AdSwitch & \textcolor{gray}{[8]} & $\textcolor{red}{\mathbf{C}'} \sqrt{T \Upsilon_T \log(T)}$ & {\Large \textcolor{orange}{$\star\star\star\star$}} & {\textcolor{orange}{€€€€}} & \textbf{none} \\
        % \hline
        Ada-ILTCB$^+$ & \textcolor{gray}{[9]} & $\textcolor{red}{\mathbf{C}'} \sqrt{T \Upsilon_T \log(T)}$ & {\textcolor{orange}{?}} & {\textcolor{orange}{€€€€€}} & \textbf{none}
    \end{tabular}
    ...
    Optimal \textbf{regret bound} is $\cR(\cA, T) \leq \textcolor{blue}{\mathbf{C}_{\Upsilon_T,\mu}} \sqrt{T \Upsilon_T} + o(\sqrt{T \Upsilon_T})$,
    with $\textcolor{blue}{\mathbf{C}} = \textcolor{blue}{\mathbf{C}_{\Upsilon_T,\mu}} = \mathcal{O}(\frac{K}{\Delta_{\text{change}}^2})$,
    %
    and $\textcolor{red}{\mathbf{C}'} \gg \textcolor{blue}{\mathbf{C}_{\Upsilon_T,\mu}}$ are much larger constants,
    ...
    \begin{small}
        \textbf{References:}
            ...
            % [2] \textcolor{gray}{[Auer et al. 02]}
            [2] \textcolor{gray}{[Garivier et al. 09]}
            % [4] \textcolor{gray}{[Garivier et al. 09]}
            [3] \textcolor{gray}{[Raj et al. 17]}
            [4] \textcolor{gray}{[Auer et al. 02]}\\
            \hspace{10pt}
            [5] \textcolor{gray}{[Liu et al. 18]}
            [6] \textcolor{gray}{[Cao et al. 19]}
            [7] \textcolor{gray}{[Besson et al. 19]}
...
\begin{frameO}[Conclusion: what we showed \hfill{} ($1/2$)]
    ...
    % \begin{colorblock}{What we showed}
        ...
        \begin{itemize}
            ...
            \item a realistic implementation on radio hardware,
            % \item my Python library SMPyBandits.
            \item theoretical results in two simplified models:
            \begin{itemize}
                \item multi-player bandits,
                \item and an extension beyond the stationary hypothesis.
            \end{itemize}
        \end{itemize}
    % \end{colorblock}
...
\begin{frameO}[Didn't have time to talk about\dots \hfill{} ($2/3$)]
    ...
    % \begin{darkblock}{Didn't have time to talk about\dots}
        ...
        \begin{itemize}
            ...
            \item and details about our algorithms, their theoretical results and proofs (Ch. 6 \& 7),
...
\begin{frame}[shrink=17]
    ...
    \textcolor{orange}{\textbf{International conferences with proceedings:}}
    \begin{itemize}
    ...
    \textcolor{darkgreen}{\textbf{Preprints:}}
    \begin{itemize}
    ...
    \textcolor{darkred}{\textbf{Submitted:}}
    \begin{itemize}
        ...
        \item ``\emph{GLRT meets klUCB \dots}'', \textbf{Besson} \& Kaufmann, for \textcolor{darkred}{AISTATS}, \textcolor{blue}{October 2019}
...
\begin{frameO}[International conferences with proceedings ($1/2)$]
    ...
    \item
        ...
        at \textcolor{orange}{\emph{IEEE WCNC}}, Marrakech, Morocco, April $2019$.\\
...
{\footnotesize
    ...
    \item
        ...
        by \textbf{Lilian Besson} \& Émilie Kaufmann,\\
        \textcolor{orange}{\emph{GRETSI}} 2019,
...
{\footnotesize
    ...
    \item
        ...
        Submitted for \textcolor{darkred}{\href{https://jmlr.org/mloss/}{JMLR MLOSS}}.\\
        \emph{See Chapter~3.}
