...
# Un problème : *Accès Opportuniste au Spectre* {.allowframebreaks}
...
# Premier algorithme pour l'OSA : $\mathrm{UCB}_1$ {.allowframebreaks}
...
- Pour $t \geq K$, à chaque étape, un *indice* numérique est calculé :
  $$ g_k(t) := \underbrace{\frac{X_k(t)}{N_k(t)}}_{\text{Moyenne empirique}\; \widehat{\mu_k}(t)} + \underbrace{\sqrt{\alpha \frac{\log(t)}{N_k(t)}}}_{\text{"Upper Confidence Bound"}\;}$$
...
# Second algorithme : Thompson Sampling {.allowframebreaks}
...
\only<1>{
\begin{block}{Exemple de regret}
Par exemple, en \emph{OSA} classique, si les bras sont ordonnés par leur disponibilité, $\mu_1 > \mu_2 \geq \dots \geq \mu_K$, , on se compare au meilleur bras $\mu_1$, et alors :
...
> - *inférieure* : pour n'importe quel problème $\mu$ d'un certain type, il existe une constante telle que **pour tout algorithme** $\mathcal{A}$
    $$\mathop{\lim\inf}\limits_{T \to +\infty} \frac{R_T^{\mathcal{A}}}{\log T} \geq C_{\inf}^{\mathcal{A}}(K, \boldsymbol{\mu}, \dots).$$
> - *supérieure* : pour n'importe quel problème $\mu$ d'un certain type, et pour **cet algorithme**, il existe une constante telle que
    $$\mathop{\lim\sup}\limits_{T \to +\infty} \frac{R_T^{\mathcal{A}}}{\log T} \leq C_{\sup}^{\mathcal{A}}(K, \boldsymbol{\mu}, \dots).$$
> - $\mathcal{A}$ est dit *optimal* pour cette famille de problème si $C_{\inf}^{\mathcal{A}}(K, \boldsymbol{\mu}, \dots) = C_{\sup}^{\mathcal{A}}(K, \boldsymbol{\mu}, \dots)$.

----

# *OSA* multi-joueurs {.allowframebreaks}
...
1. Un *rapport de recherche* (arXiv/HAL) résumant le travail de bibliographie, d'implémentation et d'expérimentation réalisé pour mon environnement de simulation (presque terminé) et la publication en libre accès du code (pour l'instant, la documentation est déjà disponible).\newline
  Cf. \urlb{http://banditslilian.gforge.inria.fr/}
...
4. Un *article* "ingénieur" exposant l'intérêt de l'agrégation d'algorithmes de bandit pour des problèmes de radio cognitive. *Objectif* : fin $2017$.
...
- Membre du bureau de l'*Association des Doctorants et Docteurs de l'IETR*\footnote{Cf. \urlb{ADDI.Asso.INSA-Rennes.fr/}} (**ADDI**) depuis février. Une douzaine d'activités prévues dans l'année (dont la Journée des Doctorants en juillet, la galette des rois de l'IETR en janvier etc).
