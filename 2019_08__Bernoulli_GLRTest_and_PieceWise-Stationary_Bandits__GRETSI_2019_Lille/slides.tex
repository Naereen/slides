...
\begin{frame}{Qu'est-ce qu'un bon test de détection de rupture ?}
  ...
  \begin{block}{1. Pas de \textbf{fausse alarme}}
    ...
    \visible<2>{
      \alert{Premier extrême} : aucune fausse alarme si on détecte \emph{jamais} \dXey{}
    }
  \end{block}
  ...
  \begin{block}{2. Un \textbf{délai de détection} court}
    ...
    \visible<2>{
      \alert{Autre extrême} : délai nul si on détecte \emph{tout le temps} \dXey{}
    }
  \end{block}
...
\begin{frame}{Le test généralisé de rapport de vraisemblances de Bernoulli (T-BGLR)}
  ...
  % \begin{block}{Le test de détection séquentielle de rupture B-GRLT}
    Le \alert{T-BGLR} est le temps d'arrêt définit par
    \begin{small}
...
\begin{frame}[plain]{Parlons d'action contre la crise climatique !}
  ...
  \begin{center}
    \includegraphics[height=0.35\textheight]{../common/TalkAboutGlobalWarmingNow.png}
  \end{center}
